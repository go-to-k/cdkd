...
const KNOWN_SCENARIOS: Record<string, string> = {
  ...
  'failed-only-journal-retention-cycle':
    'Failed-only rollback-journal retention after a CLEAN automatic rollback (issue #1208): the journal survives as reason=auto-rollback-clean / operations=[] / failedOperations=[<bad resource>] (raw S3 read), the next deploy prints the --revert-failed note, `cdkd rollback --force --revert-failed` consumes it via the #1198 skip-with-warning for a physical-id-less failed CREATE (exit 2 accepted), and a NO-CHANGE fix-forward deploy (bad resource removed) clears the journal + note (the PR #1212 no-change gap).',
