...
const KNOWN_SCENARIOS: Record<string, string> = {
  ...
  'multi-stack-outputs-only-export':
    'Outputs-only change on an already-deployed producer (issue #875): a downstream consumer starts referencing the producer, so CDK synth adds a new Output/Export to the producer WITHOUT changing any of its resources. The producer redeploy is a no-op at the resource level but must still persist the new export to state + the exports index, otherwise the consumer (deployed with --exclusively, so the producer is not redeployed to paper over the gap) fails to resolve its Fn::ImportValue. Also covers the PREVIEW half (issue #1921): `cdkd diff` must report the Outputs-only change — human Outputs section naming the `[export]` row, `--fail` exiting 1, and the entry in `--json` `outputChanges` — where it previously printed "No changes detected" and exited 0, steering the user away from the deploy that publishes the export; paired with a negative arm requiring the freshly-deployed producer to still diff clean.',
  'sdk-ccapi-crossref-boundary':
    'Heterogeneous SDK-Provider <-> Cloud Control API routing in ONE stack with `Ref` / `Fn::GetAtt` cross-references crossing the boundary in BOTH directions — an SDK-routed consumer reads a CC-routed producer, AND a CC-routed consumer reads an SDK-routed producer — against two kinds of CC-provisioned resource: a type with no SDK provider (pure Cloud Control fallback, read-only attribute via the CC read-back) and an SDK-registered type moved onto Cloud Control with `--recreate-via-cc-api`. Neither depends on a property the SDK provider leaves unhandled. Also covers the CC delete path bypassing the SDK provider delete().',

  // ---- Dynamic reference patterns ----
  'dynamic-reference-resolution':
    'CloudFormation dynamic references (`{{resolve:secretsmanager:...}}` / `{{resolve:ssm:...}}` / `{{resolve:ssm-secure:...}}`) resolved by cdkd itself (`resolveDynamicReferences`) BEFORE the property reaches the provider — JSON-key (`:SecretString:<key>`), whole-secret, and version-stage forms + plaintext SSM param; the deployed resource carries the RESOLVED value, never the literal token. Secret-bearing references are additionally REDACTED back to their expression in persisted state, decided by the parameter TYPE rather than the spelling: an `ssm:` reference to a SecureString parameter is redacted (issue #1901) while a String parameter beside it stays resolved. `ssm-secure:` is secret by SPELLING (issue #2482): its whole, embedded and `<name>:<version>` forms decrypt to the SecureString value and persist as the expression, with the parameter seeded out of band because CloudFormation cannot create a SecureString.',
  ...
  'ordinary-create-name-cooldown':
    "An ORDINARY `cdkd deploy` (no --replace, no rollback, a fresh process that never deleted anything itself) creating a resource whose NAME is still held by a prior `cdkd destroy`'s asynchronous delete, and riding the cooldown out instead of failing the deploy (issue #2116). Distinct from `rollback-reverse-replacement-name-cooldown`, which covers the sites where cdkd ITSELF just deleted the name holder and therefore consults `isRecreateRetryableError`: this one covers the reachable destroy-then-redeploy dev loop, where the only thing that can absorb the window is the generic transient classifier plus the name-cooldown backoff grid. Pass condition = the redeploy exits 0 and prints `Deployment completed successfully`; coverage is confirmed separately from a --verbose retry line quoting the AWS cooldown message, since a fully-closed window produces the same exit code. CAVEAT on reading this row as coverage: the tag is claimed unconditionally by the fixture, but the arm is OPPORTUNISTIC -- the window is a ~23s real-AWS timing property that the fixture's preceding 200s bucket poll can outlast, so a given run reports either `OK: COVERED` or `INCONCLUSIVE`, and the latter is likely the common one. Nothing forces a re-run on INCONCLUSIVE. The standing backstop for the classifier and the 64s backoff grid is the unit suite (tests/unit/deployment/retryable-errors.test.ts + retry.test.ts), which runs every time; this row records that a real-AWS arm EXISTS, not that the last run exercised it.",
  'rollback-reverse-replacement-name-cooldown':
    "`cdkd rollback` reverse-replacement re-create through the SQS same-name ~60s deletion cooldown (issue #1206): a custom-named queue rename-replacement (create-only QueueName, --force-stateful-recreation) plus an injected failing resource under --no-rollback is reverted immediately — the reverse-replacement's initial re-create must hit `QueueDeletedRecently`, retry through the window (asserted via the --verbose retry lines), restore the old-named queue, delete the new one, and exit 0.",
  'resource-type-change-replacement':
    "A resource `Type` change on an EXISTING logical id is a replacement whose two halves route on two types: the old resource is deleted through the STATE record's type and the new one created through the template's (issues #2668, #3036). Covers a stateful old type refused without --force-stateful-recreation, two types sharing one bare-name physical id, two types declaring an identical property bag (which a properties-only no-op check swallows), and both rollbacks (automatic, and `cdkd rollback` from a --no-rollback journal) re-creating each old type through its own provider. Pass condition = every old physical name confirmed gone and every new one read back from AWS, in both directions.",
  'failed-only-journal-retention-cycle':
    'Failed-only rollback-journal retention after a CLEAN automatic rollback (issue #1208): the journal survives as a SINGLE popped-and-re-recorded segment (reason=auto-rollback-clean / operations=[] / failedOperations=[<bad resource>], raw S3 read), the next deploy prints the --revert-failed note and its own clean auto-rollback PRESERVES the older failed-only segment (PR #1216, exactly 2 segments), `cdkd rollback --force --revert-failed` consumes it via the #1198 skip-with-warning for a physical-id-less failed CREATE (exit 2 accepted), and a NO-CHANGE fix-forward deploy (bad resource removed) clears the journal + note (the PR #1212 no-change gap).',
  'rollback-replay-effective-properties':
    "Reverse-replacement rollback records the replay-CREATE's `effectiveProperties` (issue #1682). Distinct from `rollback-failure-injection`, which rolls back CREATEs (a delete): here the resource is REPLACED before the failure, so rollback re-creates the OLD one from `previousState.properties` — a cdkd STATE record, which can carry a malformed block an older binary wrote and which the provider WARNS about and SUBSTITUTES (the #1544 replayWarn downgrade) rather than refusing. Pre-fix the engine typed that call's result as `{physicalId, attributes?}` and rebuilt the record from `prev.properties`, discarding the substitution. The vehicle is `AWS::EC2::Route` rather than the `AWS::S3::Bucket` #1682 names, because a bucket's reverse-replacement re-create must re-acquire a just-deleted GLOBALLY unique name whose release is not immediate (flaky for an unrelated reason), while a route's `<RouteTableId>|<Destination>` identity is stack-scoped and deterministic; `createRoute`'s multi-destination warn arm is gated on the same `CreateContext.replayingState` flag, so the engine path is identical. verify.sh deploys v1, injects a second destination key into the state record, flips the create-only destination with a failure wired AFTER the route (so rollback classifies reverse-replacement, asserted via the substitution warning), then asserts the post-rollback record restored the v1 destination and DROPPED the injected key, that two consecutive `cdkd drift` runs converge, and that destroy leaves 0 orphans.",
  'rollback-replacement-updatereplacepolicy-retain':
    "Replacement rollback under `UpdateReplacePolicy: Retain` keeps the replacement's NEW physical copy (issue #2598). A live CloudFormation A/B (us-east-1, 2026-09-05) settled which attribute governs that delete: `UpdateReplacePolicy` does (DELETE_SKIPPED, new copy orphaned OUT of the stack) and `DeletionPolicy` does not (DELETE_COMPLETE) — so the fixture carries THREE `AWS::SSM::Parameter` subjects, one per row: RetainParam (UpdateReplacePolicy Retain, no DeletionPolicy — new copy must SURVIVE), ControlParam (no policies — new copy must be DELETED), DeletionPolicyParam (DeletionPolicy Retain only — new copy must be DELETED too, the row that refutes 'DeletionPolicy governs it'). All three rename their create-only `Name` in one deploy whose failing SQS queue DependsOn all of them, so every replacement COMPLETES and the rollback classifies each as a replacement; `--force-stateful-recreation` is required because AWS::SSM::Parameter is in STATEFUL_TYPES (the Retain arm is exempt from that guard and unaffected by the flag). verify.sh asserts the deploy engine ORPHANED the old retain copy, the rollback took reverse-replacement-readopt, the new retain copy is ALIVE in SSM while both control copies are GONE, and state names every OLD copy and the survivor NOWHERE. The load-bearing assertion is on the DURABLE channel rather than the log: a rollback runs during an already-failing deploy, usually non-TTY with the log discarded, so `cdkd events --run <failed> --format json` must show the retained subject's ROLLBACK_RESOURCE_SUCCEEDED carrying the survivor's `physicalId`, a `reason` naming both ids, and `provisionedBy` — while BOTH control rows carry neither field (an unconditional physicalId would point a cleanup pass at the resource the rollback just deleted). The warn grep is kept as the interactive-user channel, with a sentinel so a reworded message fails loudly instead of going blind. Finally, `cdkd destroy` must remove the tracked copies while leaving the untracked survivor standing — which the fixture then deletes itself, since Retain means cdkd will not clean up after it. Distinct from `rollback-replay-effective-properties`, which reaches the same reverse-replacement branch but only ever in its DELETE polarity: no other rollback fixture declares UpdateReplacePolicy: Retain on a replaced resource.",
  'rollback-create-deletion-policy-snapshot':
    'DeletionPolicy: Snapshot on a ROLLED-BACK CREATE (issue #1358): a deploy whose second resource fails must snapshot the first one and DELETE it, not orphan it. verify.sh asserts the deploy fails, the old orphan log line is absent, a completed cdkd:final-snapshot-of EBS snapshot of the rolled-back volume exists, the volume is gone from AWS, and state.json is gone.',
  ...
  'cross-cutting-deploy-destroy':
    'Broad real-AWS regression set (39+ resource VPC+NAT+CF+Lambda+SQS or comparable breadth). The run to make when a change is cross-cutting rather than feature-local.',
  ...
  'cdkd-gc':
    '`cdkd gc` garbage-collection precision against real AWS: whole-bucket state-file reference scan keeps every referenced asset, an unreferenced seeded object plus an unreferenced seeded ECR image are the only deletion candidates, `--dry-run` deletes nothing, `--older-than` age guard honored (issue #1012). Since issues #1792 / #1793 the ECR half is DISCRIMINATING rather than a re-run of the plain path: two live images are referenced only through WIDENED host forms — an UPPER-cased host carrying an UPPER-cased digest (collected yet unmatchable against ECR lower-case digests before the insert-time fold, so the live image was deleted) and the dual-stack FIPS `<acct>.dkr-ecr-fips.<region>.on.aws` form the grammar had missed entirely — and both must SURVIVE the run, since a missed reference deletes irreversibly.',
...
// `escapeCell` moved to `scripts/markdown-table.ts` (issue #2636): five more
// generators had the same unescaped-prose defect this file's #2545 fix closed,
// and the escape's contract — which characters it neutralises, and the ones it
// deliberately leaves alone — is a single decision that copies drift away from
// one character at a time. The full rationale lives in that module's doc
// comment; the two call sites below keep the notes specific to THIS matrix.
