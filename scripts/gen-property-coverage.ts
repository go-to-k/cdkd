...
const CREATE_ONLY_SNAPSHOT_OUT_FILE = resolve(
  repoRoot,
  'src/provisioning/create-only-snapshot.generated.ts'
);
...
/**
 * Render `create-only-snapshot.generated.ts`: every fixture's
 * `createOnlyPropertyPaths`, the offline fallback `getCreateOnlyPropertyPaths`
 * resolves when the live `DescribeType` lookup FAILS (issue #3718).
 *
 * Every fixture, not only the Tier 1 types above: the fallback serves any type
 * cdkd holds a schema for. A fixture WITHOUT the field is refused rather than
 * skipped — skipping would silently turn that type's fallback back into "no
 * create-only properties", the miss the snapshot exists to close. Exported for
 * unit tests.
 */
export function renderCreateOnlySnapshot(
  fixtures: ReadonlyArray<{ file: string; fixture: SchemaFixture }>
): string {
  const lacking = fixtures.filter(
    ({ fixture }) => !Array.isArray(fixture.createOnlyPropertyPaths)
  );
  if (lacking.length > 0) {
    throw new Error(
      `${lacking.length} CFn schema fixture(s) carry no createOnlyPropertyPaths — ` +
        `re-capture them with scripts/refresh-cfn-schemas.mjs: ` +
        lacking.map(({ file }) => file).join(', ')
    );
  }
  const body = [...fixtures]
    .sort((a, b) => a.fixture.resourceType.localeCompare(b.fixture.resourceType))
    .map(({ fixture }) => {
      ...
      return `  [\n    ${JSON.stringify(fixture.resourceType)},\n    ${rendered},\n  ],`;
    })
    .join('\n');
  return `/**
 * AUTO-GENERATED by scripts/gen-property-coverage.ts — DO NOT EDIT BY HAND.
...
function writeCreateOnlySnapshot(): void {
  const fixtures = readdirSync(FIXTURE_DIR)
    .filter((f) => f.endsWith('.json') && !f.startsWith('_'))
    .map((file) => ({
      file,
      fixture: JSON.parse(readFileSync(join(FIXTURE_DIR, file), 'utf8')) as SchemaFixture,
    }));
  const content = renderCreateOnlySnapshot(fixtures);
  const tmp = `${CREATE_ONLY_SNAPSHOT_OUT_FILE}.tmp`;
  writeFileSync(tmp, content);
  renameSync(tmp, CREATE_ONLY_SNAPSHOT_OUT_FILE);
  console.log(
    `Wrote create-only path snapshot for ${fixtures.length} types to ` +
      `${CREATE_ONLY_SNAPSHOT_OUT_FILE}.`
  );
}
