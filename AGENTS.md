...
back into this AGENTS.md, which stays small so instruction adherence stays high.
...
approved — so the ruleset's required checks stop blocking an agent-side merge
the moment those runs are approved, and nothing else stands in the way. **The rule above — never merge
...
- **Before every commit, and before opening or merging any PR — recommended, not enforced**: run `/check` (typecheck, lint, build, tests) and `/check-docs` (README / AGENTS.md / `docs/` / `.claude/rules/` consistency with src); before a PR, run `/verify-pr`, whose checklist still applies in full — a PR whose live behavior was never exercised is not ready, whatever the unit suite says. **No hook and no marker enforce any of them any more.** The MECHANICAL merge conditions are the `main` ruleset's required checks, `integ-destroy` and `integ-schema-migration`; everything else is your own discipline, and skipping it is how main goes red. Install `vp` and markgate via `mise install` (see CONTRIBUTING.md).
...
- **Before merging ANY PR: CI must be green** — **the SERVER enforces it**: the `main` ruleset refuses the merge until its required checks pass. Still **wait with `gh pr checks <N> --watch`** (an early merge fails with a ruleset message that reads as a permissions error) and **name the PR by number, one merge per command** — a chain stops at whichever is refused, leaving the rest unrun. **The required set is not every check** — `hook-suites` notably sits outside it, so a hooks PR with a red `hook-suites` still merges. Read `gh pr checks`, not just the exit code.
- **The bash-first experiment must stay OFF**: `.claude/settings.json` pins `env.CLAUDE_CODE_THRIFTY_SONIC: "0"`. With the flag on, the session is told to read and WRITE files through `cat` / `sed -i` / heredocs instead of Read / Edit / Write, and three surfaces keyed to those tools go inert with no error line: `worktree-owner-gate.sh` (matcher `Edit|Write|NotebookEdit`), the PostToolUse `Write|Edit` → `vp run lint:fix` entry, and **every `paths:`-scoped file under `.claude/rules/`** (a rule loads only when a matching file enters context through the file tools, so a `cat`-read subsystem gets none of its notes). An explicitly set value short-circuits the server-side cohort assignment, so the pin belongs in the REPO's settings, not a maintainer's `~/.claude/settings.json`. This bullet lives in AGENTS.md rather than only in [.claude/rules/hooks.md](.claude/rules/hooks.md) because that file is itself `paths:`-scoped: when the flag is on, its explanation is invisible exactly when it is needed. Fenced by `tests/unit/scripts/settings-bash-first-optout.test.ts`.
- **The surviving safety hooks**: each blocks one foot-gun with an actionable error naming the replacement command, so the roster is NOT restated here — it is in [.claude/rules/hooks.md](.claude/rules/hooks.md). **A hook that only restates the `main` ruleset does not earn its place** — hence no commit/push or CI-green gate. **Prove a hook is ALIVE before trusting it**: `git push --dry-run origin <a merged branch>` trips `post-merge-orphan-push-gate` (`Blocked by ...`). That shape cannot probe the RULESET — the server skips rule evaluation on a dry run.
...
6. **Enforcement is procedure, not machinery.** `/check`, `/check-docs` (once per PR, at the final sha), `/verify-pr` and `/review-pr` are the recommended path and are not enforced by any hook or marker. The mechanical merge conditions are the `main` ruleset's required checks (GitHub-enforced), a verified destroy path (`integ-destroy`) and, for a state-schema bump, a verified migration run (`integ-schema-migration`). Do not add another without the maintainer's decision.
