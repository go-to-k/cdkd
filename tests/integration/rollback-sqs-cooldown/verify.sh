...
#        `--revert-failed` note at its start, and its own clean auto-rollback
#        PRESERVES the older failed-only segment (PR #1216) — the journal now
#        holds exactly 2 failed-only segments.
...
J_SEGS="$(echo "${JOURNAL_BODY}" | jq '.segments | length')"
...
# segments|length==1 pins the POP-then-re-record settlement (#1208): a
# regression that APPENDS the failed-only segment without popping the
# auto-rollback-started one would leave 2 segments here.
if [ "${J_SEGS}" != "1" ] || [ "${J_REASON}" != "auto-rollback-clean" ] || [ "${J_OPS_LEN}" != "0" ] || [ "${J_FAILED_ID}" != "FailingQueue" ]; then
  echo "[verify] FAIL: journal shape wrong (segments=${J_SEGS} reason=${J_REASON} ops=${J_OPS_LEN} failedId=${J_FAILED_ID}; expected 1 / auto-rollback-clean / 0 / FailingQueue)"
  exit 1
fi
...
# The second clean auto-rollback must PRESERVE the older failed-only segment
# (PR #1216): pop its own auto-rollback-started segment, re-record failed-only
# — leaving exactly 2 failed-only segments.
JOURNAL2_BODY="$(aws s3 cp "s3://${STATE_BUCKET}/${JOURNAL_KEY}" -)"
J2_SEGS="$(echo "${JOURNAL2_BODY}" | jq '.segments | length')"
J2_NONEMPTY_OPS="$(echo "${JOURNAL2_BODY}" | jq '[.segments[].operations | length] | map(select(. != 0)) | length')"
if [ "${J2_SEGS}" != "2" ] || [ "${J2_NONEMPTY_OPS}" != "0" ]; then
  echo "[verify] FAIL: journal after the 2nd clean auto-rollback should hold 2 failed-only segments (got segments=${J2_SEGS}, segments-with-ops=${J2_NONEMPTY_OPS}) — the older segment must be preserved (PR #1216)"
  exit 1
fi
echo "[verify] step 6 ok: the next deploy printed the failed-only --revert-failed note; older journal segment preserved (2 failed-only segments)"
