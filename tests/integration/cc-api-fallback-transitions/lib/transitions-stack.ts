 ...
 *     AWS does NOT receive `LoggingConfig`.
 *
 *   - {@link UpdateTransitionStack} (#634 item 4): a fresh deploy WITHOUT
 *     `LoggingConfig` lands the Lambda on the SDK path (state stamps
 *     `provisionedBy: 'sdk'`). A subsequent deploy where the template
 *     gained `LoggingConfig` (toggled via the
 *     `CDKD_INTEG_USE_LOGGING_CONFIG=true` env var) re-routes through CC
 *     API mid-life — state flips to `provisionedBy: 'cc-api'` and AWS
 *     now receives `LoggingConfig`. This is the more dangerous path
 ...
 * literals so `verify.sh` can `aws lambda get-function-configuration`
 ...
 *
 * Template ALWAYS emits `LoggingConfig`. The override is supplied at the
 ...
 *
 * Template emits `LoggingConfig` iff `CDKD_INTEG_USE_LOGGING_CONFIG=true`
 * at synth time. The verify.sh deploys twice:
 *   1. with the env var unset → no `LoggingConfig` → state stamps SDK
 *   2. with the env var set → `LoggingConfig` reaches the template, the
 *      diff detects the new property, `getProviderFor` returns CC, state
 *      flips from `'sdk'` to `'cc-api'`, and `LoggingConfig` lands on AWS
 */
