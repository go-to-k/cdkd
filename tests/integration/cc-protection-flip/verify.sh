#!/usr/bin/env bash
# verify.sh — cdkd generic CC protection flip integ (issue #1314).
#
# Three Tier 2 (Cloud-Control-routed) types deploy with deletion protection
# ON from creation:
...
#   2. Bare destroy (no flag). Assert: exit non-zero, all three resources
#      still exist, state file retained.
#   3. Destroy with --remove-protection. Assert: exit 0, all three
...
echo "==> Phase 1: deploy (all three types protected from creation)"
...
if [ -z "${GRAPH_ID}" ] || [ -z "${PROT_ID}" ] || [ -z "${STORE_ID}" ]; then
  echo "FAIL: missing physicalId in state (graph='${GRAPH_ID}' prot='${PROT_ID}' store='${STORE_ID}')" >&2
  exit 1
fi
echo "    graph=${GRAPH_ID} protectConfig=${PROT_ID} policyStore=${STORE_ID}"
...
if [ "${V1}" != "true" ] || [ "${V2}" != "true" ] || [ "${V3}" != "ENABLED" ]; then
  echo "FAIL: expected protection ON everywhere (graph=${V1} prot=${V2} store=${V3})" >&2
  exit 1
fi
echo "    protection is ON for all three (via cloudcontrol get-resource)"
...
for spec in "${GRAPH_TYPE}|${GRAPH_ID}" "${PROT_TYPE}|${PROT_ID}" "${STORE_TYPE}|${STORE_ID}"; do
  t="${spec%%|*}"; i="${spec#*|}"
...
echo "    all three resources survived the blocked destroy"
...
echo "==> Phase 3: destroy --remove-protection (flips all three off via CC patch, then deletes)"
...
echo "    all three resources deleted"

assert_gone "state file ${STATE_KEY} still exists after destroy" aws s3api head-object --bucket "${STATE_BUCKET}" --key "${STATE_KEY}"
echo "    cdkd state removed"

echo "[verify] PASS — generic CC protection flip (NeptuneGraph / SMSVOICE ProtectConfiguration / VerifiedPermissions PolicyStore), all 3 phases passed"
