...
# The negative half, re-keyed on the same shape so it can still FAIL. The old
# form looked for the parent name followed by a prose close-quote, which no
# code path can produce any more -- it passed vacuously.
