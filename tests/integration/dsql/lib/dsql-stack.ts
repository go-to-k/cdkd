 ...
 * value flipped to `two` — an in-place tag-only UPDATE through the CC API
 * patch-document path. verify.sh asserts the cluster identity (identifier +
 * creationTime) is unchanged, proving no replacement happened.
 */
export class DsqlStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    super(scope, id, props);

    const phase = process.env.CDKD_TEST_UPDATE === 'true' ? 'two' : 'one';
