...
- EC2 networking: `AWS::EC2::VPC` + `::InternetGateway` + `::RouteTable` (single-key) and `::VPCGatewayAttachment` + `::Route` + `::VPCCidrBlock` (composite). The `VPCCidrBlock` carries an Amazon-provided IPv6 CIDR and is the type whose absent splitter used to abort the whole command (issue [#1788](https://github.com/go-to-k/cdkd/issues/1788)); it is also what makes the IPv6 route arm reachable. The default route is what makes the `AWS::EC2::Route` splitter reachable — without it `cdkd export` aborted on every public-subnet VPC (issue [#1771](https://github.com/go-to-k/cdkd/issues/1771)).
...
- 1× `AWS::AppSync::GraphQLApi` + 1× `AWS::AppSync::ApiKey` (issue [#3414](https://github.com/go-to-k/cdkd/issues/3414)) — both re-declared by AWS in September 2026. The API is the second `COMPOSITE_PHYSICAL_ID_IDENTIFIERS` member in this fixture: CloudFormation now identifies it by `Arn` while cdkd stores the bare `apiId`, so the identifier is resolved from the recorded `Arn` attribute with an empty overlay. The key is a splitter member: cdkd stores `<apiId>|<apiKeyId>`, CloudFormation identifies it by the composite `[ApiId, ApiKeyId]` with `ApiKeyId` read-only, so the overlay narrows to `ApiId`. No `AWS::AppSync::GraphQLSchema` — that type still has no read handler, so a schema would block the export. The key's value lands in cdkd state as an attribute, which is why `verify.sh` sweeps the state prefix's S3 object versions.
...
Both the IPv4 and IPv6 destination shapes of `AWS::EC2::Route` are covered. The IPv6 arm needs the VPC to carry an IPv6 CIDR, which needs an `AWS::EC2::VPCCidrBlock` — a composite-identifier type whose missing `COMPOSITE_ID_SPLITTERS` entry used to abort this export entirely, so the fixture could not carry one until issue [#1788](https://github.com/go-to-k/cdkd/issues/1788) registered it. The remaining destination shape (`DestinationPrefixListId`) is pinned by unit tests: it needs a managed prefix list, a per-account resource this fixture would have to create and tear down for no additional splitter coverage.
