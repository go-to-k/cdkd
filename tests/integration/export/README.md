...
- No AppSync resources, deliberately (issue [#3414](https://github.com/go-to-k/cdkd/issues/3414)): a GraphQL API + key pair was added to prove the identifiers AWS re-declared in September 2026 against a real IMPORT changeset, and the run measured that CloudFormation refuses the API for IMPORT outright although its registry schema declares a read handler. The key alone imports (measured with a standalone `CreateChangeSet --change-set-type IMPORT`), but it cannot live in a stack without its API. `verify.sh` keeps the S3 object-version sweep the pair introduced; it costs nothing and closes the class for any secret-bearing resource added later.
