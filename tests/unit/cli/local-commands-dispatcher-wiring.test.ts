 ...
 * the three.
 *
 * `cdkd local start-service` + `start-alb` + `start-agentcore` +
 * `start-cloudfront` wrap cdk-local factories that call
 * `createLocalStateProvider` internally via `cdkdExtraStateProviders`
 * (registered in `local-state-source.ts`) — the ECS service emulator
 * engine for the first two, the factory's `extraStateProviders` option
 * for the latter two — so those four commands intentionally do NOT import
 * the dispatcher directly. Their wiring is covered by the shared
 * `extraStateProviders` map (verified by typecheck + the factories'
 * bundled mutual-exclusion tests in cdk-local).
 *
