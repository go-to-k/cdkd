...
describe('cdkd scrub reports a DEFERRED reference nothing resolved (issue #2157)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    synthStacks.length = 0;
    const stack = makeStackInfo('Assembled') as {
      stackName: string;
      template: CloudFormationTemplate;
    };
    // An `Fn::Sub`-ASSEMBLED secret reference whose placeholder scrub cannot
    // evaluate -- it takes no `--parameters` and this template declares none.
    // `resolveSub` warn-and-KEEPS it, so the leaf comes back carrying its
    // `{{resolve:` opening, the token scan matches nothing in it, and NOTHING
    // THROWS. That is the shape a catch-keyed finding missed entirely (round 2
    // of the #2157 review), and the reason the recorder tests SURVIVAL of the
    // raw leaf rather than the presence of an error.
    ...
    // ALREADY scrubbed, so the ONLY finding this run can produce is the
    // deferred reference. Without this the run has a real plaintext to rewrite
  ...
  it('does not report the stack clean, and --fail exits non-zero over it', async () => {
    // THE GATE THIS FENCES is the AGGREGATE one, not the per-stack one. The
    // first cut of #2157's finding wired the per-stack clean line and `--fail`
    // but left the early `return` above them untouched, so a deferred-only run
    // printed `No plaintext secrets found in any target stack state` and
    // returned BEFORE `--fail` could throw. Only a command-level case can see
    // that: `scrubStack` emits neither summary line, which is why the
    // stack-level test that was supposed to cover it could not fail.
    const err = await scrubCommand([], commandOptions({ fail: true })).catch((e: unknown) => e);

    // Not a refusal -- nothing failed, the run reached its summary.
    const errored = commandLogger.error.mock.calls.map((c) => String(c[0])).join('\n');
    expect(errored).toBe('');
    const warned = commandLogger.warn.mock.calls.map((c) => String(c[0])).join('\n');
    expect(warned).toContain('could not resolve a secret reference the intrinsics ASSEMBLE');
    const summary = commandLogger.info.mock.calls.map((c) => String(c[0])).join('\n');
    expect(summary).not.toContain('No plaintext secrets found in any target stack state');
    expect(summary).not.toContain('No plaintext secrets found in Assembled');
    expect(summary).toContain('ASSEMBLED secret reference whose region could not answer');
    // ...and `--fail` treats it as a finding, exit 1, not the exit-2 refusal.
    expect((err as { code?: string }).code).toBe('SCRUB_NEEDED');
  });

  it('NEGATIVE CONTROL: a stack with no ASSEMBLED reference is not reported', async () => {
    // Without this the case above also passes for a finding that fires on
    // EVERY stack -- the "a refusal that refuses everything satisfies every
    // positive assertion" shape. The same template with the reference spelled
    // as one complete literal defers nothing, so the recorder must stay silent
    // and the run must reach its ordinary clean summary.
    //
    // The complementary control -- a DEFERRED leaf the resolver ANSWERS -- is
    // in `scrub-cross-region-secret.test.ts`, which mocks the SDK and can
    // therefore make a lookup SUCCEED; this file cannot.
    const stack = synthStacks[0] as { template: CloudFormationTemplate };
    (stack.template.Resources!['Db']!.Properties as Record<string, unknown>)[
      'MasterUserPassword'
    ] = NAME_EXPR;
    ...
    const warned = commandLogger.warn.mock.calls.map((c) => String(c[0])).join('\n');
    expect(warned).not.toContain('could not resolve a secret reference the intrinsics ASSEMBLE');
    const summary = commandLogger.info.mock.calls.map((c) => String(c[0])).join('\n');
