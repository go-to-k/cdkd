...
describe('DynamoDBGlobalTableProvider junk previous BillingMode (issue #1552)', () => {
  ...
  it('leaves an ABSENT previous defaulting to PAY_PER_REQUEST (unchanged semantics)', async () => {
    liveTable('PROVISIONED');
    ...
    // Absent previous means the create-path default (PAY_PER_REQUEST), so the
    // desired PAY_PER_REQUEST is "no flip" — the AWS mode is deliberately NOT
    // consulted here.
    expect(billingUpdateInputs()).toEqual([]);
    expect(childLogger.warn).not.toHaveBeenCalledWith(
      expect.stringContaining('recorded previous BillingMode')
