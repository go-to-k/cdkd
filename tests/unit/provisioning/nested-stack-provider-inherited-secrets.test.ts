 ...
 * and mocks only the `DeployEngine` CLASS — the forwarding is a property of
 * that store, so a stubbed accessor could not fence it.
 */
...
import {
  withCurrentResourceSecrets,
  type DeployEngineOptions,
} from '../../../src/deployment/deploy-engine.js';
...
describe('NestedStackProvider — inherited secrets + child-region provenance', () => {
  ...
  it('records the CHILD region into the synthesized physicalId (#2055)', async () => {
    ...
    const result = await withNestedStackContext(
      makeContext({ parentRegion: 'eu-west-1' }),
      () => provider.create('Child', NESTED, {})
    );
