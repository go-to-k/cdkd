...
describe('DynamoDBGlobalTable delete retry on the index-busy refusal (issue #1830)', () => {
  ...
  it('gives up after the bounded budget instead of retrying forever', async () => {
    ...
    // and a literal 9 here would have gone on asserting the old budget — on the
    // type whose behaviour the raise MOVES.
  ...
  it('counts the REMAINING attempts in the warning, not the whole retry budget', async () => {
    ...
    // because the number was the raw `DELETE_INDEX_BUSY_MAX_RETRIES` constant —
...
describe('DynamoDBGlobalTable index-busy delete retry: wall-clock budget (issue #1830)', () => {
  ...
  it('bounds the per-retry re-arm poll instead of re-running the 15-minute one', async () => {
    ...
    // default the budget would be `DELETE_INDEX_BUSY_MAX_RETRIES` x 900s — more
    // than 3h — inside a call `destroy-runner.ts` caps at 30 min, so a genuinely
    // stuck index produced a 30-minute wait and a generic `ResourceTimeoutError`
    // that never mentions indexes, instead of AWS's own actionable sentence.
    // Bounded, the whole sequence is `DELETE_INDEX_BUSY_MAX_RETRIES` x 60s plus
    // `withRetry`'s backoff, so it SETTLES inside the window driven below;
    ...
    // backoff through the sleep seam), and still a small fraction of what the
    // UNBOUNDED shape would need — which is the discrimination this test exists
    // for. DERIVED from the budget: at a literal 700 it would have started
    // timing out the moment issue #1950 raised the retry count, failing for
    // arithmetic rather than for the bound it watches.
    //
    // What the raise DID cost here is the old comment's neater claim that the
    // whole bounded loop fits inside a single 900s poll: at 14 retries it no
    // longer does (~840s of polling). The property that discriminates is
    // unchanged — unbounded, the loop needs `budget x 900s`, over three hours.
    const driveSeconds =
      DELETE_INDEX_BUSY_MAX_RETRIES * (DELETE_INDEX_BUSY_REARM_MAX_ATTEMPTS + 8) + 60;
    expect(driveSeconds).toBeLessThan(DELETE_INDEX_BUSY_MAX_RETRIES * 900);
    await drive(() => settled, driveSeconds);

    expect(settled).toBe(true);
    await expect(outcome).resolves.toBe('rejected');
    // 1 initial + DELETE_INDEX_BUSY_MAX_RETRIES.
    expect(deleteAttempts).toBe(1 + DELETE_INDEX_BUSY_MAX_RETRIES);
  });
