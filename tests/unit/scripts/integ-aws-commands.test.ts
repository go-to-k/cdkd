...
describe('integ fixture aws invocations (#1402)', () => {
  ...
  it('parses a substantial share of the fixture tree', () => {
    ...
    // Per-SHAPE floor for the shared helpers: an aggregate over 247 verify.sh
    // cannot tell "the helper is clean" from "the helper was never read", and
    // it is the one script whose aws verbs run in eighteen fixtures at once.
    ...
    //
    // Current: 4430 invocations, 71 services, 430 verbs (measured by the
    // procedure below; ordinary corpus growth since 3903, the last +16 from
    // issue #3627's import-readback-final fixture). A stale
