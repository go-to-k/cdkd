 ...
 *
 * KNOWN SPELLINGS THE SOURCE ARMS WALK PAST (round-8 review of
 * go-to-k/cdkd#3058), recorded rather than chased: `Object['defineProperty'](`
 * (the reflective-write arm matches the dotted spelling) and
 * `(this.logger as X)['seed'] =` (the field-write count wants a dotted
 * chain). Per-spelling arms lose the race by construction; the closure is the
 * RUNTIME half in
 * `tests/unit/provisioning/secretsmanager-secret-provider-update-value-source.test.ts`,
 * which asserts every bag by value and by shape (own names, symbols,
 * prototype), and the read-side allow-list below, which needs a NAME.
 *
...
describe('shapes deliberately NOT treated as seeding, and the premises behind that', () => {
  ...
  it('the generateSecretString exemption premises still hold in the provider', () => {
    ...
    // STATEMENT position means what precedes the call (ignoring whitespace)
    // ends a statement or opens a block -- `;` `{` `}` `)` `]` or the word
    // `else` -- never a line start: a formatter breaks a long assignment
    // AFTER the `=`, so `x =\\n  this.logger.warn('')` starts a line while
    // sitting in a value slot (round-7 review, measured GREEN under the
    // line-anchored form).
    const strippedRetainBody = stripStrings(retainBody);
    const warnCalls = [...strippedRetainBody.matchAll(/this\.logger\.warn\(/g)];
    for (const call of warnCalls) {
      const before = strippedRetainBody.slice(0, call.index).replace(/\s+$/, '');
      expect(
        /(?:[;{}\)\]]|\belse)$/.test(before),
        `a this.logger.warn( in retainPreviousGenerateBlock is in a VALUE position (preceded by ` +
          `\`${before.slice(-12)}\`) — its return value can carry a stash into the bag; re-open #2212`
      ).toBe(true);
    }
    expect(warnCalls.length, 'the helper lost its drop warning; see the unit suite').toBe(1);
