...
describe('work-issues launch-mode probe', () => {
  ...
  const assertSection5Unconditional = (implement: string): void => {
    ...
    expect(
      ...
      /(only|just|if|when|unless|matters|applies|needed|necessary)[^.]{0,60}(detached|already merged)/,
    );
    // ...and the CONDITIONAL-SENTENCE shape, which neither pin above reaches:
    // a reviewer's probe found `Branch in place only if the branch here is
    // detached or its PR has already merged.` passing both (the exact pin
    // needs the comma, the reword pin needed one of four verbs). Any sentence
    // putting a branching verb in the same breath as the withdrawn condition
    // is the regression, however it is worded.
    expect(
      prose,
      `references/implement.md makes branching in place CONDITIONAL on the withdrawn ` +
        `condition. Whatever the wording, a lane that branches only sometimes commits onto ` +
        `the outer tool's branch the rest of the time.`,
    ).not.toMatch(/(branch|switch|commit)[^.]{0,80}(detached|already merged)/);
  ...
  describe('the section 5 pins discriminate every shape a re-introduction can take', () => {
    ...
    const shapes: Array<[string, string]> = [
      ...
      [
        'a CONDITIONAL sentence with none of the reword pin\'s verbs',
        'Branch in place only if the branch here is detached or its PR has already merged.',
      ],
      [
        'the same with `unless`, wrapped',
        'Take a fresh branch here unless the branch\nhere is detached or its PR has already merged.',
      ],
      [
        'a leading `When ...` clause',
        'When the branch here is detached, switch to a fresh branch before committing.',
      ],
    ];
...
describe('work-issues section 2 worktree probe', () => {
  ...
  const SINGLE_COMMIT = new RegExp(
    String.raw`\bgit\b.*(?:(?<![-\w])show\s+.*${ONE_COMMIT_FLAGS}` +
      String.raw`|(?<![-\w])log\b(?=.*(?:\s-1\b|\s-n\s+1\b|\s--max-count[= ]1\b))(?=.*\s${ONE_COMMIT_FLAGS}))`,
  );
  ...
  const visibleProse = (markdown: string): string[] =>
    (marked.parse(markdown, { async: false }) as string)
      ...
      // `<script>` / `<style>` bodies render as nothing, and an element the
      // author marked invisible (`hidden`, `display:none`) renders as nothing
      // too -- a reviewer's probe satisfied the floor with all four. Dropped
      // WITH their content, unlike an ordinary tag. The bound stated rather
      // than claimed away: this is a CARRIER LIST after all, only a much
      // shorter one than a source scan needs, and text hidden by a CSS class
      // defined elsewhere is invisible to it -- cdkd's docs ship no stylesheet,
      // so that shape has nowhere to come from today.
      .replace(/<(script|style)\b[\s\S]*?<\/\1>/gi, '')
      .replace(/<([a-z][a-z0-9]*)\b[^>]*(?:\shidden(?=[\s/>])|display:\s*none)[^>]*>[\s\S]*?<\/\1>/gi, '')
      .replace(/<!--[\s\S]*?-->/g, '')
  ...
  const proseFloorHolds = (markdown: string): boolean =>
    visibleProse(markdown)
      // Not a command: a rendered line that IS a bare git call explains
      // nothing. It must be the WHOLE line, though -- a reviewer's probe
      // showed the first spelling dropped `\`git show --stat HEAD\` reads ONE
      // commit, which is why the range exists.`, a legitimate explanation that
      // happens to open with the backticked command. Sentence punctuation is
      // the discriminator: a bare command carries none.
      .filter((line) => !(GIT_LINE.test(line) && !/[.!?](\s|$)/.test(line)))
  ...
  describe('the floor reads RENDERED text, so an invisible carrier cannot satisfy it', () => {
    ...
    const carriers: Array<[string, string]> = [
      ...
      ['a hidden span', '<span hidden>show --stat HEAD</span>'],
      ['a display:none div', '<div style="display:none">show --stat HEAD</div>'],
      ['a script body', '<script>var x = "show --stat HEAD";</script>'],
      ['a style body', '<style>/* show --stat HEAD */</style>'],
    ...
    it('is satisfied by a sentence that OPENS with the backticked command', () => {
      // The control for the command-line exclusion: a rendered line starting
      // with a git call is prose when it carries a sentence, and a command
      // when it does not. The first spelling of the filter dropped both.
      expect(
        proseFloorHolds(
          `${stripped}\n\n\`git show --stat HEAD\` reads ONE commit, which is why the range exists.\n`,
        ),
      ).toBe(true);
      expect(proseFloorHolds(`${stripped}\n\n\`git show --stat HEAD\`\n`)).toBe(false);
    });
  ...
  describe('the ban sees a prose continuation and every semantic sibling', () => {
    ...
    it('does not report the ranged probe or `git log` over a range', () => {
      ...
      // A REF whose name ends in `-1` is not `-1` the flag (reviewer probe).
      expect(banned('git log --stat feat-1')).toEqual([]);
      expect(banned('git log --raw v1-1..v1-2')).toEqual([]);
      expect(banned('git log --stat HEAD~1')).toEqual([]);
