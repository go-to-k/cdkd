...
describe('nested-stack scope of the flags (#2567)', () => {
  ...
  it('refuses on the STATE type when the template disagrees', () => {
  ...
  it('prints the PLAIN row when state and template agree — the common nested case', () => {
    // The rendered row has two shapes and only one was pinned. Measured:
    // relaxing the combined-form condition to `templateType !== undefined`
    // stayed 86/86 green while shipping "AWS::CloudFormation::Stack in state,
    // AWS::CloudFormation::Stack in the template" for every ordinary nested
    // row — the case a user actually hits.
    const v = validateRecreateTargets({
      template: parentTemplate(),
      state: st('S', { ChildStack: res(NESTED_TYPE, { physicalId: 'arn:child' }) }),
      recreateViaCcApi: ['ChildStack'],
      allowUnsupportedProperties: new Set(),
      forceStatefulRecreation: false,
    });
    const error = renderRecreateTargetsErrors(v) ?? '';
    expect(error).toContain('- ChildStack (AWS::CloudFormation::Stack)');
    expect(error).not.toMatch(/in state,.*in the template/);
  });

  it('REFUSES when only the TEMPLATE says nested — the delete routes on that type', () => {
    // This case was pinned the OTHER way for one round, on a reviewer's
    // reasoning that `NestedStackProvider` does not own a row the state records
    // as ordinary, so no cascade could happen. A later security review measured
    // the opposite and the code agrees: a Type change emits an UPDATE carrying
    // `resourceType: desiredResource.Type` (`analyzer/diff-calculator.ts`), the
    // engine binds `const resourceType = change.resourceType`, and
    // `oldDeleteProvider` resolves from THAT — so the delete goes to
    // `NestedStackProvider.delete` and destroys `<parent>~<logicalId>` whole.
    // Refusing on either type is what closes both destructive paths.
    const v = validateRecreateTargets({
    ...
    expect(v.blockedNestedStackTargets.map((t) => t.logicalId)).toEqual(['WasOrdinary']);
    const error = renderRecreateTargetsErrors(v);
    expect(error).toMatch(/refuses to operate on 1 nested-stack resource/);
    // The row names BOTH types, because refusing a target the state calls an
    // SNS topic reads like a mistake without the template half.
    expect(error).toContain('AWS::SNS::Topic in state, AWS::CloudFormation::Stack in the template');
  });
