...
cdkq と CloudFormation のデプロイ速度を比較するベンチマークスクリプトです。
...
- AWS 認証情報が設定済みであること
- Node.js >= 20.0.0
- cdkq がビルド済みであること（`npm run build`）
...
```bash
SKIP_CDKQ=true ./tests/benchmark/run-benchmark.sh
```
...
| 変数 | 説明 | デフォルト |
|------|------|-----------|
| `STATE_BUCKET` | cdkq の状態管理用 S3 バケット | 自動解決（`cdkq-state-{accountId}-{region}`） |
| `AWS_REGION` | AWS リージョン | `ap-northeast-1` |
| `CDKQ_BIN` | cdkq バイナリのパス | `./dist/cli.js` |
| `SKIP_CFN` | `true` で CloudFormation ベンチマークをスキップ | `false` |
| `SKIP_CDKQ` | `true` で cdkq ベンチマークをスキップ | `false` |
...
cdkq は CloudFormation をバイパスし、Cloud Control API を直接使用してリソースをプロビジョニングします。これにより以下のオーバーヘッドが排除されます:
...
cdkq は DAG ベースの並列実行により、依存関係のないリソースを同時にデプロイすることも可能です。
