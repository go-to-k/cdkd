...
description: "The rules an SDK provider follows — error handling, pre-flight refusal, idempotency, removal semantics, attribute return, drift read-back, and property coverage."
unlisted: true
---

# Provider implementation rules

The rules an SDK provider follows, and why each exists. Most were written after
a defect got past review, so each carries the shape of the failure it prevents
rather than only the rule.

This page is for someone writing or reviewing a provider. Start from
[Provider Development](provider-development.md), which walks the interface and
the steps end to end; come here for the decisions each step implies.
...
- [Provider Development](provider-development.md) — the interface, the examples,
  and the steps to add a provider
- [Integration fixture conventions](integ-fixture-conventions.md) — the rules a
  fixture exercising a provider follows
