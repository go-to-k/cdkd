...
4. **Custom Resource 非同期/SNS/SFN** (優先度: 中) → 詳細: §4 タスク17

5. ~~**ロールバック機構**~~ → ✅ 実装済み（--no-rollback フラグ）

6. **IAMロール置換時の削除改善** (優先度: 中)

7. ~~**Fn::GetAZs**~~ → ✅ 実装済み（全組み込み関数サポート完了）

8. **Progress bar / UI** (優先度: 低)
...
1. **Custom Resource 非同期/SNS/Step Functions 対応** (優先度: 中)
   - SNS バックエンド、Step Functions バックエンドは未対応
   - 実装場所: `src/provisioning/providers/custom-resource-provider.ts`

2. **IAMロール置換時の削除改善** (優先度: 中)
   - 削除失敗時にエラーを投げる / 孤立リソースの追跡
   - 実装場所: `src/provisioning/providers/iam-role-provider.ts`

3. **Progress bar / UI** (優先度: 低)
