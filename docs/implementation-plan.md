...
2. **統合テストの UPDATE 動作テスト環境整備** ✅ 完了
   - ✅ 基本的なUPDATE動作確認済み（basic スタックでタグ追加）
   - ✅ Cloud Control API の JSON Patch (RFC 6902) 動作確認済み
   - ✅ 環境変数でUPDATEテスト切り替え機能実装済み
     - `CDKQ_TEST_UPDATE=true` でUPDATEテスト用のコード変更を有効化
     - コード変更なしでUPDATEテストを再実行可能
     - 実装場所: `tests/integration/examples/basic/lib/basic-stack.ts`
   - ✅ Lambda スタックでUPDATE動作確認済み（IAM Role/Policy + Lambda更新）
...
6. **IAMロール置換時の古いリソース削除改善** (優先度: 中)
   - 削除失敗時のエラーハンドリング強化
   - 孤立リソースの追跡

7. **Progress bar / UI** (優先度: 低)
   - デプロイ進捗の可視化
...
1. **Custom Resource の CloudFormation 統合修正** (最優先)
   ...
   - Lambda 関数からの CloudFormation レスポンス URL への接続問題
   - 実装場所: `src/provisioning/providers/custom-resource-provider.ts`
   - 影響: CDK の autoDeleteObjects 機能が動作しない（multi-resource テスト失敗）
