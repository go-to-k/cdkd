...
#### 16. ARN Builder ユーティリティの共通化

**課題**: ARN 構築ロジックが `intrinsic-function-resolver.ts` と `cloud-control-provider.ts` に分散。
...
- [ ] 既存の ARN 構築コードをユーティリティに移行

**対象リソース**: DynamoDB, S3, IAM Role/Policy, Lambda, SQS, SNS + 今後追加されるもの
...
**統合テスト実行結果** (2026-03-25時点):

| テスト名 | スタック名 | 操作 | 状態 | 備考 |
| --------- | ----------- | ------ | ------ | ------ |
| basic | CdkqBasicExample | CREATE | ✅ 成功 | S3バケット作成 |
| basic | CdkqBasicExample | UPDATE | ✅ 成功 | タグ追加、JSON Patch動作確認 |
| conditions | CdkqConditionsExample | CREATE | ✅ 成功 | AWS::NoValue対応完了 |
| parameters | ParametersStack | CREATE | ✅ 成功 | パラメータ処理、BucketName指定 |
| intrinsic-functions | CdkqIntrinsicFunctionsExample | CREATE | ✅ 成功 | Ref, GetAtt, Join, Sub動作確認 |
| lambda | LambdaStack | CREATE | ✅ 成功 | DynamoDB + IAM + Lambda (初回) |
| lambda | LambdaStack | UPDATE | ✅ 成功 | IAM Role/Policy + Lambda更新 (3リソース更新) |
| cross-stack-references | CdkqExporterStack | CREATE | ✅ 成功 | Fn::ImportValue動作確認 |
| cross-stack-references | CdkqConsumerStack | CREATE | ✅ 成功 | Fn::ImportValue完全動作確認 |
| multi-resource | CdkqMultiResourceExample | CREATE | ❌ 失敗 | Custom Resource問題（詳細下記） |

**既知の問題**:

1. **multi-resource: Custom Resource エラー**
   - エラー: `getaddrinfo ENOTFOUND pre-signed-s3-url-for-response`
   - 原因: Custom::S3AutoDeleteObjects の Lambda 関数が CloudFormation レスポンス URL に接続できない
   - 影響: CDK の autoDeleteObjects 機能が動作しない
   - 対処: Custom Resource Provider の CloudFormation 統合修正が必要
   - ファイル: `src/provisioning/providers/custom-resource-provider.ts`

### 🚧 未実装・優先度高

1. **Custom Resource の CloudFormation 統合修正** (最優先)
   - Lambda 関数からの CloudFormation レスポンス URL への接続問題
   - pre-signed URL の解決とレスポンス送信の実装
   - 実装場所: `src/provisioning/providers/custom-resource-provider.ts`

2. **CLI の利便性向上** (優先度: 高)
   - `--state-bucket` の環境変数対応: `CDKQ_STATE_BUCKET` から読み込み
...
5. **IAMロール置換時の古いリソース削除改善** (優先度: 中)
   - 削除失敗時のエラーハンドリング強化
   - 孤立リソースの追跡

6. **Progress bar / UI** (優先度: 低)
   - デプロイ進捗の可視化

7. **Bootstrap デフォルトバケット名** (優先度: 高) → 詳細: §4 タスク15
   - `cdkq-state-{accountId}-{region}` で自動生成
   - バケットポリシーでアカウント制限

8. **ARN Builder ユーティリティ共通化** (優先度: 中) → 詳細: §4 タスク16
   - `src/utils/arn-builder.ts` 新規作成
   - 分散した ARN 構築ロジックの統合

9. **Custom Resource 非同期/SNS/SFN 対応計画** (優先度: 中) → 詳細: §4 タスク17
   - ResponseURL の実装（最優先サブタスク）
   - SNS・Step Functions バックエンドは将来対応

10. **統合テスト大幅拡充** (優先度: 高) → 詳細: §4 タスク18
    - ECR 統合テスト、API Gateway、E2E フルサイクル等

11. **対応機能/リソース表の README 追加** (優先度: 高) → 詳細: §4 タスク19
    - 組み込み関数、リソースタイプ、制限事項の一覧表

12. **公開判断基準の定義** (優先度: 高) → 詳細: §4 タスク20
    - v0.1.0 リリース条件とインパクト最大化施策
