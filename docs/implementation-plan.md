...
- [x] ~~Fn::FindInMap, Fn::Base64~~ → 実装済み（2026-03-26）
- [x] Fn::GetAZs ✅ 実装済み（2026-03-26）
...
- **CREATE リトライ** — IAM伝播遅延に対応するエクスポネンシャルバックオフ
...
1. ~~**multi-resource 統合テスト再検証**~~ → ✅ 完了（ResponseURL 修正済み）

2. ~~**統合テスト拡充**~~ → ✅ 完了（ECR, API Gateway, E2E, ECS Fargate — 合計10例）

3. ~~**リソース属性解決の強化**~~ → ✅ 完了（Phase A + B: CC API フル活用 + CFn Registry Schema 自動発見）

4. ~~**Custom Resource SNS対応**~~ → ✅ 実装済み（SNS Topic ServiceToken 検出 + Publish）

5. ~~**ロールバック機構**~~ → ✅ 実装済み（--no-rollback フラグ）

6. ~~**IAMロール削除改善**~~ → ✅ 実装済み（ポリシーdetach、インスタンスプロファイル除去）

7. ~~**Fn::GetAZs**~~ → ✅ 実装済み（全組み込み関数サポート完了）

8. ~~**Progress bar / UI**~~ → ✅ 実装済み（[1/N] ✅ 形式 + operation type表示）

9. ~~**公開判断基準**~~ → ✅ ほぼ完了（CI/CD, CLI/cdk.json, bootstrap デフォルト, 環境変数 対応済み）

**AWS実行検証結果** (2026-03-26):

| テスト | Deploy | Destroy |
|--------|--------|---------|
| ECS Fargate (15リソース) | ✅ 15/15 | ✅ 14/15 |
...
1. **Custom Resource Step Functions 直接統合** (優先度: 低)
...
3. **統合テスト追加候補** (優先度: 中)
   - CloudWatch Alarms / SNS Subscription / Step Functions

4. **`npm install -g cdkq` でインストール可能** (優先度: 高)
   - package.json の最終調整、npm パブリッシュ設定
...
- Custom Resource の SNS/Step Functions バックエンドは未対応
