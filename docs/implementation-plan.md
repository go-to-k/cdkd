...
**実装**: `tests/integration/examples/` (合計19例)
...
**課題**: ~~現在 Lambda 同期呼び出し（`RequestResponse`）のみ。ResponseURL がダミー値。~~ → Phase A, B 実装済み。Step Functions のみ未対応。
...
- Step Functions バックエンドは未対応（非常にレア）
...
- [x] SDK Provider 対応リソース (9種: IAM Role, IAM Policy, S3 BucketPolicy, SQS QueuePolicy, EventBridge Rule, API Gateway Account, API Gateway Resource, API Gateway Deployment, API Gateway Stage)
- [x] カスタムリソース対応状況
- [x] 未対応機能一覧
...
- Lambda バックエンドのカスタムリソースは対応済み（Step Functions は未対応）
...
- **統合テスト** — 合計19例（basic, lambda, ecr, apigateway, multi-resource, ecs-fargate, cloudwatch, stepfunctions, dynamodb-streams, eventbridge, s3-cloudfront, sns-sqs-event, ec2-vpc, rds-aurora, bedrock-agent, conditions, parameters, intrinsic-functions, cross-stack-references）
...
- **属性解決 Phase B** — CFn Registry Schema 自動発見
...
- **S3 + CloudFront OAI**: OAI (OriginAccessIdentity) の S3CanonicalUserId 属性取得に制約あり。CC API が返さないため手動フォールバックが必要
- **AgentCore (Bedrock Agent) IAM 伝播**: IAM Role 作成直後の Bedrock Agent 作成で IAM 伝播遅延エラー + already-exists エラーが発生する場合がある
- **API Gateway Lambda Permission 削除**: API Gateway 削除時に Lambda Permission の削除が先行すると、統合が壊れた状態で残る場合がある（依存順序の問題）

**AWS実行検証結果** (2026-03-26):

| テスト | Deploy | Destroy |
|--------|--------|---------|
| ECS Fargate (15リソース) | ✅ 15/15 | ✅ 14/15 (VPC依存順序で1件失敗) |
...
2. **ECS Fargate destroy の VPC 依存順序修正** (優先度: 中)
   - destroy 時に1リソース削除失敗（VPC依存リソースの逆順削除が不完全）

3. **パフォーマンスベンチマーク** (優先度: 低)
   - CloudFormation vs cdkq の速度比較データ
   - 大規模スタック（100+リソース）のデプロイ時間計測

4. **Custom Resource Step Functions 直接統合** (優先度: 低)
   - State Machine ARN を ServiceToken に指定するケース（非常にレア）
...
- Custom Resource の Step Functions バックエンドは未対応（非常にレア）
- ECS Fargate destroy 時に VPC 依存リソースの削除順序が不完全（1件失敗）
