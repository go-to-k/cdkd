...
cdkd は CDK CLI (`aws-cdk`) を**置き換える**のではなく、**デプロイ方法の代替**を提供します。
...
- cdkd は既存の CDK アプリケーションをデプロイするツールであり、プロジェクト生成は責務外
- **推奨ワークフロー**:
  1. `npx aws-cdk init app --language typescript` で CDK プロジェクト作成
  2. CDK アプリを実装 (aws-cdk-lib を使用)
  3. `cdkd deploy` で高速デプロイ

#### `cdk bootstrap` について

**結論**: cdkd 独自の `bootstrap` コマンドを実装済み

| 項目 | CDK CLI | cdkd |
|------|---------|------|
| **コマンド** | `cdk bootstrap` | `cdkd bootstrap` |
...
- **cdkd**: 状態管理用 S3 バケットのみを作成 (CloudFormation 不使用のため IAM ロール不要)

**アセットの扱い**:

- CDK CLI: bootstrap で作成した専用バケットにアセットをアップロード
- cdkd: `@aws-cdk/cdk-assets-lib` を使用し、CDK が合成時に決定したバケット/キーにアップロード
...
- [x] parameters example (CloudFormation Parameters)
- [x] conditions example (CloudFormation Conditions)
...
- [x] bedrock-agentcore example (AgentCore Runtime)
...
- すべて英語で記述（実装計画書のみ日本語）
- SDK Provider 優先 + Cloud Control API フォールバックを明記
...
- [x] デフォルトバケット名の自動生成: `cdkd-state-{accountId}-{region}`
...
- [x] 対応組み込み関数の一覧表
- [x] Cloud Control API 対応リソースの代表例
- [x] SDK Provider 対応リソース (`src/provisioning/providers/` 参照)
- [x] カスタムリソース対応状況
- [x] 未対応機能一覧
...
- [x] `npm install -g cdkd` でインストール可能
...
```bash
cdkd deploy MyStack                           # 位置引数（cdk.json から --app 自動読み込み）
```

**実装内容**:

- [x] `--stack-name` → 位置引数（`cdkd deploy [stacks...]`）に変更
- [x] シングルスタックの自動検出（スタックが1つなら指定不要）
- [x] `--all` フラグで全スタックデプロイ
- [x] ワイルドカード対応（glob パターン）
- [x] `--app` を省略可能に: `cdk.json` の `app` フィールドから自動読み込み
- [x] `--state-bucket` を省略可能に: `cdk.json` の `context.cdkd.stateBucket` から読み込み
- [x] 環境変数対応: `CDKD_STATE_BUCKET`, `CDKD_APP`
...
- [x] セキュリティ監査
  - [x] 依存パッケージの脆弱性スキャン — npm audit 実施。cdkd 自体に脆弱性なし（devDependency/CDK内部のみ）
...
- 頻繁に使用されるリソースタイプに SDK Provider を実装済み
- 未対応リソースは CC API にフォールバック
...
**cdkd** は、CDK エコシステムとの完全な互換性を保ちながら、CloudFormation のボトルネックを解消する革新的なツールです。
...
- **CLI スタック指定 CDK 互換化** — 位置引数 `cdkd deploy MyStack`、`--all`、ワイルドカード
- **Bootstrap デフォルトバケット名** — `cdkd-state-{accountId}-{region}` + ポリシー + 暗号化 + バージョニング
...
- **`--state-bucket` 自動解決** — STS アカウントIDから `cdkd-state-{accountId}-{region}` を自動生成
...
- **統合テスト** — `tests/integration/` 以下の全例（basic, lambda, ecr, apigateway, multi-resource, ecs-fargate, cloudwatch, stepfunctions, dynamodb-streams, eventbridge, s3-cloudfront, sns-sqs-event, ec2-vpc, rds-aurora, bedrock-agentcore, conditions, parameters, intrinsic-functions, cross-stack-references, cloudfront-function-url, custom-resource-provider, multi-stack-deps, composite-stack, full-stack-demo）
...
- **ユニットテスト** (SDK Provider + CC API フォールバック)
...
1. **`npm install -g cdkd` でインストール可能にする** (優先度: 高)
   - package.json の最終調整、npm レジストリへのパブリッシュ設定

2. **パフォーマンスベンチマーク** (優先度: 低)
   - CloudFormation vs cdkd の速度比較データ
