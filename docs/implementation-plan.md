...
- すべて英語で記述（実装計画書のみ日本語）
- SDK Provider 優先（34種）+ Cloud Control API フォールバック（200+ リソースタイプ）を明記
...
- [x] SDK Provider 対応リソース (34種: `src/provisioning/providers/` 参照)
- [x] カスタムリソース対応状況
- [x] 未対応機能一覧
...
- 頻繁に使用されるリソースタイプ（34種）に SDK Provider を実装済み
...
- **ユニットテスト 291件** (SDK Provider 34種 + CC API フォールバック)
