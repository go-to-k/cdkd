...
are installed with pnpm 12 through Vite+.
...
Which verification a PR needs follows from the paths it touches. Two rows are
enforced by a merge gate — their path lists are the `integ-destroy` and
`integ-schema-migration` scopes in
[`.markgate.yml`](https://github.com/go-to-k/cdkd/blob/main/.markgate.yml),
which is the source of truth for both. The rest are expected of the
