...
| `RESOURCE_STARTED` / `RESOURCE_SUCCEEDED` / `RESOURCE_FAILED` | Per per-resource CREATE / UPDATE / DELETE (carries logicalId, resourceType, `provisionedBy`, duration, and error metadata on failure). `physicalId` is present whenever cdkd knows one — always on success, and on failure for an UPDATE / DELETE (whose target already had a state record) or for a CREATE that got far enough to materialize its resource (issue [#2169](https://github.com/go-to-k/cdkd/issues/2169) — e.g. an ACM certificate whose `ISSUED` wait timed out). It is absent only for a CREATE that failed before AWS produced an id, where there is nothing to name. |
