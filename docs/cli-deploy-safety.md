...
The same walk refuses an absolute `aws:asset:path` inside a nested template,
which would point outside the assembly directory (on a row of the stack's own
template it is already refused when the assembly is read), and a tree nested more than 512 levels
