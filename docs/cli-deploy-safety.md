...
hand-modified or generated by a non-CDK toolchain. Refusing to deploy any level
of it.
```
...
the check does not wait until the walk reaches the repeat. Any of the parent
stack's own resources already created in the same run are rolled back as for
any failed deploy.
