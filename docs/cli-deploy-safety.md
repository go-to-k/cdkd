...
`Metadata['aws:asset:path']`. Before cdkd deploys a nested stack it walks every
template reachable from that row, and refuses when a path resolves to a
template already on the same nesting chain:

```text
NestedStackProvider: The nested template tree under stack 'Parent' contains a
cycle: 'Child' (/path/to/cdk.out/child.json) -> 'Loop'
(/path/to/cdk.out/child.json). Nested stack 'Loop' resolves to a template that
is already on that nesting chain, so its Metadata['aws:asset:path'] closes a
cycle. CDK emits an acyclic nested template tree with relative asset paths, so
this indicates the synth output was hand-modified or generated by a non-CDK
toolchain. Refusing to deploy any level of it.
```
...
The whole subtree is checked before the first nested level deploys, however
deep the cycle sits, so no level of a cyclic tree is provisioned and no child
state record is written. Each nested level is a real deployment, which is why
the check does not wait until the walk reaches the repeat. The parent stack's
own resources that were already created in the same run are rolled back as for
any failed deploy.

The same walk refuses an absolute `aws:asset:path` at any depth, which would
point outside the assembly directory.
