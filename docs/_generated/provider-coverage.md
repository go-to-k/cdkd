---
title: "Provider coverage report"
unlisted: true
---

# Provider Coverage Report

Auto-generated by `scripts/audit-provider-coverage.ts`. Do not edit by hand; re-run the script to regenerate.

- Generated: `2026-07-31T06:56:37.717Z`
- Schema version: 1
- Total CFn resource types audited: **1705**

## Summary

| Tier | Provisioning path | Count |
|------|-------------------|-------|
| **Tier 1** | SDK Provider (preferred) | 134 |
| **Tier 2** | Cloud Control API fallback | 1372 |
| **Tier 3** | Not provisionable by cdkd today | 199 |

## Tier 1 — SDK Provider registered

Direct SDK calls via `src/provisioning/register-providers.ts`. Fastest, drift-aware, and shape-stable on reads. This is the canonical path; new high-traffic types should land here.
...
## Tier 2 — Cloud Control API fallback

No dedicated SDK Provider, but `ProvisioningType` is `FULLY_MUTABLE` or `IMMUTABLE`. cdkd routes these through the generic Cloud Control API today — deployable, but slower (async polling) and shape-divergent on reads (see `CC_API_FALLBACK_DENY_LIST` for known drift offenders). Frequency-weighted picks from this list are the next candidates for dedicated providers.
