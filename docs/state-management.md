...
> `a|b` in database `mydb` would be recorded as `mydb|a|b`, which a plain split
> reads as database `mydb`, table `a` — both halves non-empty, so nothing
> downstream can tell it is wrong. Rather than record an ambiguous id,
