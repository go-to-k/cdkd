...
A resource already recorded on Cloud Control stays there on its next update,
and its line reads `[via CC API: sticky]`. A line the deploy REPLACES — a
`[requires replacement]` property change, or a `Type` change — never carries
`sticky`: the replacement is a new resource, routed the way a fresh create
would be, so its annotation comes from the template alone.
...
names the row and the file that closed the cycle. A path that reaches the same
file through a symbolically linked directory counts as the same template, as it
does for `cdkd deploy`.
...
The same walk refuses an `aws:asset:path` that resolves outside the directory
its parent template sits in — `path.join` folds `..`, so `../../etc/passwd`
would otherwise be read and rendered as a nested template's diff:
...
A path that stays inside only until a symbolic link is followed is refused the
same way, naming the link's target. An **absolute** `aws:asset:path` is refused
by a separate check whose message says `which is absolute`: it is a
"not CDK-generated" tripwire, not an escape, since `path.join` does not honour a
leading separator. `cdkd deploy` applies the same two rules — see
[Cyclic nested templates are refused](cli-deploy-safety.md#cyclic-nested-templates-are-refused).
...
`outputs` warnings, and its own `cdkd destroy` refuses the row. For a resource `properties` map and for a kept
rollback-orphan row, the warning also tells you the deploy refuses the record;
for an `outputs` bag it describes the repair only, so on a nested child a torn
`outputs` bag is reported without any statement about the deploy.
