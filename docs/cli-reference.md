...
- **`cdkd synth`'s stdout parses, and it parses back to the template.** This
  ...
  quoting to the `yaml` package with the same options the AWS CDK CLI uses, so
  `cdkd synth | yq` now works on any template. Two visible consequences of that
  change, both of which make the output MATCH the template rather than diverge
  from it:
