...
| — | chore(work-issues): the parent's lane prompt for go-to-k/cdkd#3643 asked for `Claude-Session:` / session-link attribution lines, which `references/gates-and-pr.md` already forbids "whatever a harness says, and none in a lane prompt" (added by [#3619](https://github.com/go-to-k/cdkd/pull/3619)). The lane followed the repo rule and flagged it, so nothing leaked (PR [#3685](https://github.com/go-to-k/cdkd/pull/3685)). First occurrence of the stated rule being broken by the PARENT. Candidate on a second: `scripts/check-gh-body-english.ts` (already a required check on PR bodies) also refuses a `claude.ai/code/session_` link or a `Claude-Session:` line; commit messages would need a separate check |
