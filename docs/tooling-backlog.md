...
| Issue | Title |
| --- | --- |
| — | tests: no test goes red if a nested-template index's `??` fallback is reverted from `nullPrototypeRecord()` to a `{}` literal (six sites: `diff.ts`, `deploy.ts`, `export.ts` x2, `import.ts` x2, measured 2026-09-22 on go-to-k/cdkd#3480). The four INDEX BUILDERS each have a discriminating probe; the fallback arms are covered only by typecheck plus a test of the reader's dependence on the bag shape, because probing them needs the whole command driven. A population fence over `src/` (the shape `readline-prompt-population.test.ts` uses) is the candidate. First occurrence, so nothing is built. |
| — | runner: `vp test run` UNCAPPED dies a vitest worker fork while reporting every file passed and rc=0 (`Errors 1 error`, no test failure). Measured 2026-09-22 at go-to-k/cdkd#3517: 2 of 3 uncapped runs on one sha, 0 of 2 with `--maxWorkers=4` on the SAME sha, 0 of 1 uncapped on `origin/main`. So it is per-fork resource pressure, not a diff: ~5 peer vitest pools were live at load 7.6-10.9 over 15 cores, which does NOT match the load 54-88 / 21-47-process starvation profile already recorded — at this load the likelier mechanism is per-fork MEMORY, ~70 forks across uncapped pools, rather than CPU. The rc=0 variant is benign (the runner attributed no failure and a dead worker's files are re-run); the rc=1 signature is the one that matters. What is missing is a way to tell the two apart without reading the log, since rc alone cannot — MEASURED at go-to-k/cdkd#3517: the same crash, all 1116 files passing either way, came back rc=0 on two runs and rc=1 on a third, so the exit code does not even separate the variants reliably. Capped runs stayed clean across four attempts at two different shas (load 7.6-10.9, 4-5 peer pools). First occurrence at this load profile, so nothing is built. |
| — | rules: the `.claude/rules/**` TOTAL is over its 300 KB budget. Measured 325,368 B on 2026-09-22 at go-to-k/cdkd#3514, which itself adds 2,514 B to an already-over total (~322 KB measured during go-to-k/cdkd#3506; ~320 KB at that PR's base). So the trim this row records is ~2 KB larger than when it was filed, and each PR that documents an invariant here grows it again. No fence enforces the total, only the per-file caps, which are all clear. First occurrence, so nothing is built — the trim is somebody's whole-layer pass, not a line in an unrelated PR. |
| — | test(integ): `schema-v8-to-v9-migration` is STALE against the current binary. Its v9 phase asserts `state.version == 9` after deploying with the working-tree cdkd, which now writes v10, so the run FAILs at `producer (v9 deploy) state.version is 10, expected 9` before its later assertions. Measured on go-to-k/cdkd#3658, where AWS was clean after the failure. Its last PASS (2026-08-26) predates the v10 bump. The fixture needs to pin a v9 binary for that phase, or be retired now that `schema-v9-to-v10-migration` exists. First occurrence, so nothing is built. |
| — | test(integ): a `verify.sh` failure path that runs `destroy --force` unconditionally destroys a PEER's live stack when the failure was lock contention. Observed 2026-09-26 on go-to-k/cdkd#3817's merge-gate run: another workspace took `CdkdDriftRevertExample`'s lock seconds after a clean pre-flight, this run's deploy failed on the lock, and `drift-revert`'s cleanup then deleted 12 of the peer's live resources once its lock released. A fixture whose deploy fails with the lock-contention refusal should skip its destroy. SECOND occurrence, agent-side: on go-to-k/cdkd#3813's `custom-resource-noecho-nested` run a peer's destroy removed the stack mid-run, this run's destroy was refused on the peer's lock, and `/run-integ` step 7's auto-cleanup then deleted the peer's SSM parameters, roles, layers, state records and exports-index keys. The agent half SHIPPED by [#3827](https://github.com/go-to-k/cdkd/pull/3827) as a step 7 exclusion (nothing is deleted while a peer runs the fixture); the `verify.sh` half stays a row |
...
| [#2089](https://github.com/go-to-k/cdkd/issues/2089) | chore(work-issues): a watchdog cancel that fires the watchdog, a pgrep that matches itself, and the one-file revert that attributes an integ failure — the first item SHIPPED with the orphaned-`sleep` row below (no cancel exists to fall through to `WATCHDOG_FIRED`) |
| — | chore(run-integ): the `( sleep N; kill ) &` watchdog leaves its `sleep` ORPHANED when the run finishes — `kill $wpid` reaps the subshell, not its child, and the surviving `sleep` inherits the driver's stdout, so a piped driver's output is withheld until the full timeout (measured twice this session: a finished run read as still running for ~47 min). Related to #2089, different half — SHIPPED by the 2026-09-24 work-issues retro (the watchdog now polls in 5s sleeps and exits on its own, so nothing kills it), on its second occurrence |
| — | chore(run-integ): on a watchdog FIRE, `kill -9 $VPID` kills only the `bash verify.sh` process, so its `node dist/cli.js deploy` / `destroy` child survives reparented to PID 1 and keeps calling AWS while steps 6-11 clean up. Measured by the retro PR's reviewer with a forking stand-in (2 children survived per run on bash 3.2, 5.3 and zsh), never hit on a real run. Candidate: start verify in its own process group and fire `kill -9 -- -$VPID`. First occurrence, so nothing is built. SECOND occurrence, on a real run (2026-09-25): the fixed 1500s budget fired on `dynamodb-gsi-update` (normal ~1300s, that run 1503s in its designed index-busy wait) and the orphaned destroy kept running; a 2700s re-run passed. SHIPPED by the 2026-09-26 work-issues retro: the recipe starts verify under `perl -e 'setpgrp(0,0); exec @ARGV'` (zsh refuses `set -m` outside a terminal) and fires `kill -9 -- -$VPID`, and derives the budget from the ledger's last duration (2x, floor 1500s) |
...
| — | test(scripts): [#3384](https://github.com/go-to-k/cdkd/issues/3384) states its population as an ENUMERATED table of eight suites, six of them unbounded, and the row above is already one hand-widening of it. Derived on `main` today, `grep -lE 'spawnSync\|execFileSync' tests/unit/scripts/*.test.ts` returns 41 suites and 33 declare no `}, N);` per-case bound — so the eight is stale by the same mechanism that row describes, and `withretry-interrupt.test.ts` (2 spawns, 0 bounds) is a second file outside it, surfaced by timeouts reported on go-to-k/cdkd#3499's branch. Extending the list by hand is what has happened twice; the remedy is to state the population as the derivation |
| — | chore(hooks): `integ-destroy-gate.sh` scopes from the WORKTREE's current branch rather than from the branch of the PR named on the command line, so `gh pr merge <N>` run while standing on another branch is judged against that other branch's diff. Observed 2026-09-24: a docs-only PR was refused for "touches deletion logic" because the worktree sat on a PR whose diff reached `destroy-runner.ts`; checking out the docs branch and re-running the same command passed. Same family as [#3365](https://github.com/go-to-k/cdkd/issues/3365) (the gates judging the wrong pull request) but a different trigger — that one is the URL selector in the PR-number walk, this one is the scope computation ignoring the selector entirely. Second occurrence of the family; the fix is to resolve the scope from the named PR's head, not from `HEAD` |
| — | test(scripts): `docs-error-strings.test.ts` sits in [#3384](https://github.com/go-to-k/cdkd/issues/3384)'s table as an exemplar of the fix — 6 spawns against 11 declared bounds — and still times out under a loaded run. That is not the missing-bound shape but go-to-k/cdkd#2953's, whose note at `docs-error-strings.test.ts:604-614` concludes "Declaring a timeout would have fixed the symptom; the scan itself was the redundancy". The file makes 8 `runOnCopy` spawns over a ~19 MB copied tree plus 2 whole-repo spawns, so a bound is not its remedy; sharing one copied tree and one parse is. Second occurrence of that shape |
...
| — | chore(rules,skills): the numeric per-file and per-corpus budgets were dropped from AGENTS.md's Tooling Policy, because nothing measured them and both corpora had already drifted past them unnoticed. What remains is the prose rule (keep each file small; a change that grows one trims it in the same PR) plus the down-only byte ceiling on AGENTS.md, which IS measured. If a numeric budget comes back it needs a test, not a sentence — measure a corpus with `find <dir> -type f -exec wc -c {} +`, since `du -sk` reads disk blocks and overstates it |
| [#2648](https://github.com/go-to-k/cdkd/issues/2648) | test(integ): a fixture whose subject is the silent-drop auto-route rots when its trigger property is back-filled — `cc-api-fallback-transitions` went RED that way when go-to-k/cdkd#1621 made its `RuntimeManagementConfig` trigger HANDLED, the same rotted premise go-to-k/cdkd#3454 and go-to-k/cdkd#3457 repaired in `recreate-via-sdk-provider`, `recreate-mixed-direction` and `sdk-ccapi-crossref` (rule: `docs/integ-fixture-conventions.md`, "Never seed a Cloud Control route from an unhandled property"). The fix for go-to-k/cdkd#2648 moved it to `AWS::ApiGatewayV2::Api.Body` and gave it and `sdk-to-cc-autoroute` `cc-api-fallback`'s step-0 coverage-table guard, so the next back-fill reds with its own diagnosis at run time. A unit test reading each fixture's named `Type:Property` trigger against `property-coverage.generated.ts` would catch it offline, before any run, but needs fixtures to DECLARE their trigger first |
...
| — | chore(work-issues): nothing says to locate the SEAM a design question's options need, and check it against the files open PRs hold (`claim.md`'s open-PR stand-down), BEFORE asking the maintainer that question for a same-run pickup. On go-to-k/cdkd#3573 the maintainer chose "absorb the legacy baseline at comparison time"; the lane then found the only seam was `src/cli/commands/drift.ts`, held by open fork PR go-to-k/cdkd#3486, and stood down before any edit — one maintainer decision and one dispatch spent on a pickup that rule refuses. §2's collision map cannot see it: a fork PR's files appear in no local worktree diff, and `drift.ts` is not on the cross-cutting list. Candidate: one clause on `references/gotchas.md`'s "A lane needing a user decision goes through `AskUserQuestion`" bullet; no step for "reclassify `now` after a maintainer answer" exists yet. SECOND occurrence on the same issue: a later run posted a QUEUED claim on #3573 after checking only its triage file-hold list, while open PR go-to-k/cdkd#3586 had grown to hold `drift.ts`; the claim was edited into a stand-down. The probe half SHIPPED by [#3619](https://github.com/go-to-k/cdkd/pull/3619) (`triage.md` §2's `gh pr list` now reads `files`, re-run before every later claim); the ask-before-locating-the-seam half stays a row. Same stale-triage-evidence family, third shape: a run promoted QUEUED claims on go-to-k/cdkd#3700 / #3704 / #3627 after peers had closed them — SHIPPED by the 2026-09-25 work-issues retro as a state-and-premise re-check before promotion in `claim.md` §4 |
| — | chore(work-issues): a lane subagent's `tail -F` log watcher or background integ re-fires an interim "agent stopped with background work still running" task-notification to the parent on each output line — about 20 no-op notifications from one lane, each costing the parent a turn with nothing to act on. SECOND occurrence in the next run (~25 from one lane waiting on its own background work) — SHIPPED by [#3686](https://github.com/go-to-k/cdkd/pull/3686): `implement.md` §5-g now has a lane wake on its background run's exit, never on a per-line watcher |
| — | chore(work-issues): a lane flattened with `git reset --soft origin/main` instead of `ship.md` §9's `git reset --soft "$(git merge-base origin/main HEAD)"`, after a fetch had moved `origin/main` past the lane's base. `--soft` keeps the index, so the commit got the NEW main as parent and the OLD base's tree plus the lane delta — a silent revert of every peer merge in between. On go-to-k/cdkd#3611 it deleted, among others, go-to-k/cdkd#3605's `ec2-volume-delete.ts`, its test, hunks in `cloud-control-provider.ts` and `wait-abandoned.ts`, two integ `verify.sh` edits, its changelog entry and its generated-doc rows; CI was green and four reviewers plus a second-opinion pass read only the intended delta. Caught before merge by `git diff --stat origin/main...HEAD` listing files the lane never touched; rebuilt by applying `git diff <old base> <bad commit>` onto `origin/main`. `gates-and-pr.md` §7 already prints that exact merge-base `--stat`, and it DOES show the reverted files, so the candidate is a clause there to read it after every flatten for paths outside the lane's scope, not a new probe. SECOND occurrence on go-to-k/cdkd#3772 (the parent session, same deviation, caught by a 34-file staged count): SHIPPED by go-to-k/cdkd#3790 as a `git show --stat` line right after the recipe's commit in `ship.md` §9 |
| — | chore(work-issues): lane reports contradicted two stated rules and only the parent's reading caught them. Two lanes whose change reached the `cdkd local` container path reported "no integ needed", against AGENTS.md's cost-is-not-a-tiebreaker rule; the parent ran the `local-*` fixtures. A lane filed its cross-repo sibling (go-to-k/cdk-local#749) as `Session-fit: next` because "this lane holds a cdkd tree only", which is neither of `session-report.md`'s two valid reasons; the parent reclassified it `now`. First occurrence of each, so nothing is built. The `next` half RECURRED three times in one run (three go-to-k/cdkd#3515 rows deferred as "an umbrella owns it", "a different shape", "shares a file with another path") — SHIPPED by the 2026-09-25 work-issues retro as a `filing.md` §5-f clause naming those as not external input |
| — | chore(work-issues): a population grep written into a filed umbrella row matched ONE spelling of the shape. go-to-k/cdkd#3515's removal-diff row gave `if \(!\(\w+ in \w+\)\)`, which misses `.filter((k) => !(k in next))`; the next lane's review found 9 more sites (fixed in go-to-k/cdkd#3784). Candidate: one clause on `filing.md`'s fold recipe — a row that states a grep states it over every spelling. First occurrence, so nothing is built |
| — | chore(work-issues): §3-0's 60-minute fresh-issue window let go-to-k/cdkd#3585 through at 66 minutes while its filer's session was still live; triage's manual "the filer may take it" note was the only guard. The run QUEUED it, the filer claimed it 44 minutes later, and the run stood down with no work lost. First occurrence, so nothing is built |
| — | chore(work-issues): a maintainer-imposed hard stop mid-run is named by neither `claim.md`'s stand-down template nor `session-report.md`'s `next` reasons, and "nothing about the SESSION is a reason" reads as forbidding it. A run stood down every queued issue, reclassified a `now` (go-to-k/cdkd#3631) to `next` citing the stop, and back to `now` when the maintainer resumed the same session 3h later; the existing text was enough to do it. First occurrence, so nothing is built |
| — | chore(run-integ): integ serialization is per-RUN only (`ship.md` §9 grants one lane at a time), and a fixture deploys a FIXED stack name (`CdkdLocalRunTaskFromStateFixture`), so a second session running the same fixture shares its stack, state and lock. A `/work-issues` run needed `local-run-task-from-state` for go-to-k/cdkd#3651 while another workspace's serial `local-*` sweep included it; only an ad-hoc process-list check caught it before the run. Candidate: `/run-integ` step 4 checks for a live runner on the same fixture (process list, plus the fixture stack's state lock) and waits. SECOND occurrence on go-to-k/cdkd#3787: `remove-protection` failed on a peer session's live lock on its stack; polled until it cleared, re-ran clean. SHIPPED by go-to-k/cdkd#3790 as a `/run-integ` step 4 clause: a `lock.json` under the prefix with a future `expiresAt` is a live peer, wait then re-scan |
| — | chore(work-issues): the parent's lane prompt for go-to-k/cdkd#3643 asked for `Claude-Session:` / session-link attribution lines, which `references/gates-and-pr.md` already forbids "whatever a harness says, and none in a lane prompt" (added by [#3619](https://github.com/go-to-k/cdkd/pull/3619)). The lane followed the repo rule and flagged it, so nothing leaked (PR [#3685](https://github.com/go-to-k/cdkd/pull/3685)). First occurrence of the stated rule being broken by the PARENT. SECOND occurrence: a parent's lane prompts shipped the link in six merged PR bodies (#3584 ... #3658, cleaned by hand), and a third put one in the go-to-k/cdkd#3512 lane brief (the lane refused) — SHIPPED by [#3734](https://github.com/go-to-k/cdkd/pull/3734): `scripts/check-gh-body-english.ts` now also refuses a `claude.ai/code/session_` link or a line-leading `Claude-Session:` in titles and bodies. Commit messages stay unchecked (the squash message is the PR title plus a BLANK body). The 2026-09-25 run's lane prompts asked again (lanes refused); its retro MOVED "none in a lane prompt" from `gates-and-pr.md`, which only lanes read, to `implement.md` §5-g's lane-prompt guardrails |
| — | chore(work-issues): the skill assumes `origin` is go-to-k/cdkd, and a run from a contributor FORK (`origin` = the fork, issues off) breaks three steps silently. Every `gh issue` / `gh pr` call needs `-R go-to-k/cdkd`; the fork PR's CI stays `action_required` until the maintainer approves it, so §8's CI-green wait cannot finish and the merge is the maintainer's; and `ship.md` §9's IN-PLACE `git -C "$MAIN" pull origin main` pulls the FORK's `main`, which lagged upstream and lacked the merge, so the rebuild would have shipped the old binary. The go-to-k/cdkd#3720 run pulled `https://github.com/go-to-k/cdkd.git main` `--ff-only` instead, and branched the retro off upstream `FETCH_HEAD`. SECOND occurrence (go-to-k/cdkd#3774 run: triage listed an empty backlog off the fork before switching to `-R`): the launch-mode probe now prints `ORIGIN` and its fork paragraph adds the `upstream` remote. The IN-PLACE `pull origin main` in `ship.md` §9 stays a row |
| — | chore(work-issues): IN-PLACE, every lane shares ONE tree, so the `integ-destroy` gate's `hash: diff` marker reads whichever branch that tree holds. Merging a docs-only PR (go-to-k/cdkd#3770) while the tree already held the next lane's `src/**` delta (go-to-k/cdkd#3759) was refused for that other delta's missing integ; it merged once that lane's integ set the marker. `ship.md` §9 names the marker's tree for the MAIN-CHECKOUT case only. Candidate: one IN-PLACE line there — merge each PR before switching the tree to the next lane's branch, or merge by number from a tree on the PR's branch. First occurrence, so nothing is built |
| — | chore(run-integ): `/run-integ` takes one fixture or `all`, never a named SET, so a PR needing 14 fixtures (go-to-k/cdkd#3786) was driven through a scratch runner the parent wrote: serial, a per-stack state and lock check before and after each fixture, a watchdog, one ledger row each. It worked, and recorded the pre-existing `alb` FAIL ([#2085](https://github.com/go-to-k/cdkd/issues/2085)). First occurrence, so nothing is built |
| — | chore(work-issues): a parent told its lanes to wait for WHOLE-BUCKET quiescence (no `lock.json` anywhere in the state bucket, nothing modified for 10 minutes) before any integ, stricter than `/run-integ` step 4's live-lock-on-THIS-fixture rule. The account is shared with other hosts' sweeps and never went quiet: one lane waited ~1h through repeated refusals, another hit its 2h cap and timed out; the per-fixture check the skill already states is what worked (go-to-k/cdkd#3762). The text was right, so nothing is built |
| — | chore(scripts): a FUTURE-dated integ-ledger row wins `normalize-integ-ledger.ts`'s newest-row dedupe, so a real later run of that test is silently deleted and the commit reads "nothing to commit". First seen 2026-08-14 (a JST date filled as midnight UTC, ~7h ahead); SECOND on 2026-09-25, when a lane hand-wrote 08:20Z at 07:29Z and the parent's correct row vanished. SHIPPED by the 2026-09-26 work-issues retro: the normalizer refuses, in both modes and before dedupe, a row more than 300s past the current clock |
| — | chore(work-issues): the parent adopted a reviewer nit — gate `glue-update-hardening`'s log-group sweep on `state destroy` rc==0 — without checking its premise. `cleanup()`'s `state destroy` runs AFTER the fixture has already destroyed the stack, so it exits non-zero on every SUCCESS; the sweep was skipped and a log group leaked until the change was reverted. A cleanup trap's rc is not the fixture's destroy result. `verify.md` §8-h already says "a right finding can carry a wrong FIX: check the PREMISE", so this is a stated rule broken once; nothing is built |
| — | chore(work-issues): mid-run the harness reaped a lane's single-file vitest run for low memory. The cause was two vitest process trees from ANOTHER workspace at 100% CPU since 2026-09-16, found with `ps aux \| grep '[v]itest'` and killed on the maintainer's go-ahead. `gates-and-pr.md` §6's host-load check counts vitest processes but does not say to read their START TIME and tree for a stale peer. First occurrence, so nothing is built |
| — | chore(lint): inserting a function between an existing JSDoc block and its declaration orphans the doc (the LAST block before a declaration wins). THIRD occurrence: twice on go-to-k/cdkd#2213 (2026-08-25), and on go-to-k/cdkd#3829, where `isLockInfoExpired` landed between `LockManager`'s class doc and the class. Mechanically detectable (`*/` followed only by whitespace and `/**`), but measured 2026-09-26 that shape has 118 sites in `src/**` and nobody has classified which are orphans and which are deliberate (a section banner above a documented declaration, say), so a fence needs that population triaged first — a whole-tree pass, not a retro line |
