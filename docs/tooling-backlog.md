...
| — | chore(work-issues): nothing says to locate the SEAM a design question's options need, and check it against §2's held-file list, BEFORE asking the maintainer that question for a same-run pickup. On go-to-k/cdkd#3573 the maintainer chose "absorb the legacy baseline at comparison time"; the lane then found the only seam was `src/cli/commands/drift.ts`, held by open PR go-to-k/cdkd#3486, and stood down before any edit — one maintainer decision and one dispatch spent on a pickup §2 would have refused. Candidate: one clause at the step that reclassifies an issue `now` after a maintainer answer. First occurrence, so nothing is built |
