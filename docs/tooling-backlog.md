...
| — | skill(work-issues): the PARENT put a `Claude-Session:` trailer and session link in a lane brief (2026-09-25, lane for go-to-k/cdkd#3512), though `gates-and-pr.md` §6 forbids one "in a lane prompt". The lane refused, so nothing shipped with it. The rule sits in a lane-read file, while the parent's dispatch checklist (`implement.md`, "Guardrails every lane prompt must carry") does not repeat it. First occurrence, so nothing is built. |
