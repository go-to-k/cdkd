...
import { createLocalStartAgentCoreCommand } from './local-start-agentcore.js';
...
const CDKD_EMBED_CONFIG = {
  ...
  // AWS_IAM SigV4, opt IN to fail-closed via `--strict-sigv4`. The previous
  // cdkd-divergent fail-closed default (security review #484) is dropped in
  // this PR to keep the shim surface consistent with upstream — see the
  // `feedback_shim_blocked_by_unadopted_semantic_divergence.md` memory rule
...
async function localInvokeCommand(target: string, options: LocalInvokeOptions): Promise<void> {
  ...
  //
  // Lines are MOVED, not suppressed. ONE mechanism this does NOT reach, a
  // raw write that never passes through cdkd's logger — so a consumer piping
  // this command still needs `| tail -1` for a BUILT image today:
  // **cdk-local's own `ConsoleLogger`**, a SECOND logger module with its own
  // state and no reservation concept — every shim under `src/local/` that
  // re-exports cdk-local (`buildContainerImage`, `buildAgentCoreCodeImage`,
  // `downloadAndExtractS3Bundle`) emits its `logger.info` straight to
  // `console.info` ([#2429](https://github.com/go-to-k/cdkd/issues/2429)).
  // This is why a container-image Lambda still prints `Building container
  // image (platform=...)` on stdout.
  //
  ...
  // try/finally pair: `resolveImagePlan` runs `mkdtempSync` + the layer merge
...
interface ImagePlan {
   ...
   * host by merging (`copyLayerTreeLastWins`) each layer's asset directory into one
...
export async function resolveZipImagePlan(
  ...
  // download + unzip BEFORE the same merge path runs, so the
 ...
 *     `materializeLambdaLayers` merge path. AWS's "last layer
 ...
 *      `copyLayerTreeLastWins`), then bind-mounts the merged tmpdir at
...
export function materializeLambdaLayers(layers: { logicalId: string; assetPath: string }[]): {
  ...
  try {
    for (const layer of layers) {
      ...
      // `copyLayerTreeLastWins` (issue #3106); `local-start-api.ts`'s merge
      // is the same call.
...
export function createLocalCommand(): Command {
  ...
  local.addCommand(createLocalStartAgentCoreCommand());
