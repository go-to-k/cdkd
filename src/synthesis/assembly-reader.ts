...
export class AssemblyReader {
  ...
  private buildAssetManifestMap(
    ...
    for (const [artifactId, artifact] of Object.entries(manifest.artifacts)) {
      ...
      if (props?.file) {
        ...
        if (!resolved.contained) {
          throw new SynthesisError(
            `Asset manifest artifact ${displayStackName(artifactId)} has ` +
              `file=${displayAssemblyPath(props.file)} which ` +
              `${renderAssemblyPathEscape(resolved, assemblyDir)}`
          );
        }
  ...
  private extractStackInfo(
    ...
    if (!templateFile) {
      throw new SynthesisError(`Stack ${displayStackName(stackName)} has no templateFile property`);
    }
    ...
    if (!resolvedTemplate.contained) {
      throw new SynthesisError(
        `Stack ${displayStackName(stackName)} has templateFile=${displayAssemblyPath(templateFile)} ` +
    ...
    if (dependencyNames.length > 0) {
      this.logger.debug(
        `Stack ${displayStackName(stackName)} depends on: ` +
    ...
    for (const [logicalId, resource] of Object.entries(template.Resources ?? {})) {
      ...
      if (
        ...
        throw new SynthesisError(
          `Stack ${displayStackName(stackName)} nested-stack ${displayIdent(logicalId)} has ` +
