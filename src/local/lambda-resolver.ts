 ...
 * `isAbsolute(p) ? p : resolve(dir, p)`. A second hand-written copy is how a
 * guard on one twin becomes a guard on neither — the argument
 * `resolveFileAssetSourcePath` and `resolveDockerContextDirectory` each
 ...
 *   change: both sites mounted the named directory verbatim. Every
 *   `aws:asset:path` a real `cdk synth` emits is a relative `asset.<hash>`, so
 *   what this costs is a hand-written assembly or a non-CDK toolchain, each of
 *   which gets a refusal naming the value.
 ...
 * stages a Stage's assets into the APP's outdir while the Stage's manifest sits
 * in `cdk.out/assembly-<Stage>/`. Binding to the manifest directory refuses
 * every Stage asset as "hand-modified" — go-to-k/cdkd#3493's B1, fixed there
 * and re-provable here by the Stage acceptance cases in
  ...
  if (isAbsolute(assetPath)) {
    throw wrapError(
      `Lambda '${displaySafe(logicalId)}' has ` +
        ...
        `CDK emits asset paths relative to the assembly directory; an absolute one indicates ` +
        `the synth output was hand-modified or generated by a non-CDK toolchain. cdkd ` +
        `bind-mounts this directory into the container, so honouring it would expose an ` +
        `arbitrary directory on this machine to the function's code. Refusing to mount it.`
    );
  }
 ...
 * {@link resolveAssetCodeDirectory} is one function: go-to-k/cdkd#3493's own
 * defect was two sites disagreeing about the BOUND, which no refusal test can
 * see.
 *
 ...
 * filename gives the base. The fallback to `process.cwd()` is for a stack with
 * no asset manifest at all, which in practice no Lambda-bearing stack is.
 *
 * `assetOutdir` is the app's outdir and is the CONTAINMENT bound. An ABSENT one
 * falls back to the base, which is correct for a top-level stack and NARROWS
 * for a Stage — it never opens past the base, so a hand-built `StackInfo`
 * carrying neither field is refused rather than admitted. `AssemblyReader`
 * always sets it.
 *
...
export function assetPathDirs(stack: StackInfo): {
  ...
  const manifestDir = stack.assetManifestPath ? dirname(stack.assetManifestPath) : process.cwd();
