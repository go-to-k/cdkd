...
export interface RunEcsTaskOptions {
   ...
   * boot via `resolveHostGatewayExtraHosts()` (in `local-run-task.ts`) and
   * threaded down here — merged into the verbatim `--add-host` flag list
   * alongside {@link addHostFlags}. Empty / undefined → no mapping (Docker
   * Desktop resolves `host.docker.internal` natively; Linux native dockerd
   * needs this since 20.10). Distinct from {@link addHostFlags}, which
   * carries the Cloud Map peer-discovery entries. Mirrors cdk-local #483.
   */
  ...
  /**
   * ECS analogue of the Lambda-container fix shipped in PR #670:
...
export async function runEcsTask(
  ...
  // distinct name so its order vs the peer entries is irrelevant.
...
interface BuildDockerRunArgs {
  ...
  /**
   * ECS analogue of the Lambda-container fix in PR #670 — bind-mounts
...
export function buildDockerRunArgs(opts: BuildDockerRunArgs): {
  ...
  // PR #670 follow-up — bind-mount the host-side profile credentials
  ...
  // that targets `/cdkd-aws/credentials` doesn't shadow the creds
  // file — docker honors mount-order by container path uniqueness;
  // a later mount at the same target would fail or shadow.
  ...
  // PR #670 follow-up — point the container's SDK chain at the bind-
  ...
  // Split the resolved env into `-e` flags plus the values that must travel
