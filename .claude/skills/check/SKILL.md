...
It is scope-dependent, which is what makes it confusing rather than merely surprising: in the same measurement `docs` stayed rc=0 throughout (`tests/**` is not in its scope), while a new file under `docs/` stales BOTH. So the same action stales one gate, both, or neither depending only on where the file landed. (The original measurement's third case — three untracked files under `.claude/hooks/` leaving both gates at rc=0 — no longer holds: `.claude/hooks/**` joined the `check` scope in issue #2381, so that path now stales `check`.)
