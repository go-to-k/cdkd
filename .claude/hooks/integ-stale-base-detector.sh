...
scope_re='^src/provisioning/providers/|^src/provisioning/(cloud-control-provider|region-check|provider-registry|register-providers)\.ts$|^src/cli/commands/(destroy|destroy-runner|deploy)\.ts$|^src/deployment/(deploy-engine|retry|retryable-errors|rollback-executor|intrinsic-function-resolver)\.ts$|^src/analyzer/(dag-builder|template-parser|implicit-delete-deps|lambda-vpc-deps)\.ts$'
