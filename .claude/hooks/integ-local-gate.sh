...
# test as complete.
# The add-only shape cannot occur (cdkd already depends on cdk-local, so a
# `+` with no `-` would mean re-adding it after a removal); case 20d2 pins
# 0 there. `minus` / `plus` are not reset per file block either, so a `-` in
# one manifest and a `+` in another fires it -- over-refusing, the safe
