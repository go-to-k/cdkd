...
live_tree_state() {
  ...
  printf ' dirty=%s' "$(git -C "$SCRIPT_REPO" status --porcelain 2>/dev/null | wc -l | tr -d '[:space:]')"
}
...
EOF
      ;;
    slow-graphql)
      # Sleeps past `gate_bounded`'s 5s alarm. The hook must come back with the
      # commits-only count instead of hanging into the harness's 15s kill, which
      # would fail the gate OPEN.
      #
      # `exec sleep`, NOT `sleep`. A forked child outlives the alarm that kills
      # its parent and keeps the command substitution's pipe open, so the caller
      # waits the FULL sleep with the bound working perfectly — measured 20s
      # bounded and 20s unbounded, which made this case blind to the very thing
      # it exists for. Real `gh` is one process (measured: a 1s alarm bounds a
      # live `gh api graphql` to 1s, rc 142), so `exec` is also the faithful
      # model.
      exec sleep 12
      cat <<'EOF'
{"data":{"repository":{"pullRequest":{"timelineItems":{"nodes":[]}}}}}
...
run_case() {
  ...
  if [ "$got" = "$want" ] && [ "$live_now" = "$LIVE_BASELINE" ] && [ "$gq_ok" -eq 1 ] && [ "$tier_ok" -eq 1 ] && [ "$time_ok" -eq 1 ]; then
    pass=$((pass + 1))
    printf 'OK   %s (exit %s)\n' "$name" "$got"
  else
    fail=$((fail + 1))
...
# 14m. The timeline query sleeps past `gate_bounded`'s alarm. Unbounded, this
#      would run into the harness's own 15s kill, which produces NO exit 2 —
#      i.e. the gate fails OPEN. Bounded, it degrades to the commits-only count.
run_case "a hanging timeline query is bounded, not left to the harness kill" 0 \
  slow-graphql stale "" \
  "gh pr merge 2634" yes "" 9
...
run_case "the timeline query names this PR's owner/repo/number" 2 \
  flattened-fixback stale "" \
  "gh pr merge 2634" "-F number=2634" 1-reviewer
