...
# The sentinel is seeded EXPLICITLY here rather than inherited from the case
# above: a reviewer measured that this case reddened under the fallback
# mutation only because the previous case had failed to claim, which is state
# dependence, not discrimination. Seeded, it asks its own question.
printf 'sessA %s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" > "$WGD/session-owner"
