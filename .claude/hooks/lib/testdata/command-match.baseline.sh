# command-match.baseline.sh -- VENDORED GOLDEN FILE, NOT A LIVE HOOK LIBRARY.
#
# This is `.claude/hooks/lib/command-match.sh` exactly as it stood BEFORE the
# go-to-k/cdkd#2027 work, kept as the baseline for the differential fence in
# `../command-match-differential.test.sh`.
#
# PROVENANCE
#   commit: 8e84e4e20d6b7e957683ac6d8f69e21a77f1534f
#   path:   .claude/hooks/lib/command-match.sh
#   sha256: 2af09c64246d12cc4d9dab1bf14c8cbc8d16da738ddaa012877e68a4e30f6e85   (of the vendored content below the marker line)
#   regenerate:
#     git show 8e84e4e2:.claude/hooks/lib/command-match.sh
#
# WHY VENDORED rather than read from git. The fence used `git show <sha>` and CI
# shallow-clones, so the object was absent and the fence refused to run -- which
# was the correct behaviour (it declined to report a pass it had not earned) but
# meant it never ran in CI at all. A golden file is hermetic: no history, no
# network, no clone depth, and it survives a squash or a history rewrite that
# renumbers the SHA.
#
# DO NOT EDIT the content below. It is a snapshot of a past generation, not code
# anyone maintains; the fence verifies its sha256 and fails loudly on a
# mismatch. Nothing sources this in production -- it is loaded only by the
# differential fence, and it lives under `testdata/` so neither
# `run-tests.sh` (globs `lib/*.test.sh`) nor the class fence (globs the hooks
# directory) picks it up.
#
# ===== VENDORED CONTENT BEGINS BELOW THIS LINE - DO NOT EDIT =====
#!/usr/bin/env bash
