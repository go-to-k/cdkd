...
edits={
 ...
 # go-to-k/cdkd#3204. `redir-leader` deletes the leading-redirection strip,
 # so one token in front of a guarded verb silences every gate again;
 # `redir-operator-split` puts the segmenter back to splitting on the `&`
 # of `2>&1` and the `|` of `>|`, which breaks the verb's command position
 # even with the strip in place -- the two halves fail independently and
 # each has its own cases.
 'redir-leader':       ('    if [[ ! "$s" =~ $_gate_redir_heredoc ]] && [[ "$s" =~ $_gate_redir ]]; then\n      s="${BASH_REMATCH[3]}"\n    fi\n', ''),
 'redir-operator-split': ('          if ((c == "&" && (substr(line, i - 1, 1) ~ /[<>]/ || substr(line, i + 1, 1) == ">")) ||\n              (c == "|" && substr(line, i - 1, 1) == ">")) continue\n', ''),
