...
exact shape. **The allow-list is EMPTY** as of the PR 2 sweep (issue #2388): every client
site spreads the helper first. Emptiness is asserted, not merely permitted --
every other assertion about the list (sorted, deduplicated, names existing files)
passes vacuously once it is empty, so re-adding an entry needs a rationale in the
JSON `$comment` and a reviewer. The ratchet direction below is what got it there.

**What makes the completeness claim MECHANICAL is the reconciliation, not the
green run.** An empty allow-list only says the checker found nothing; it says
nothing about how much the checker can SEE. That distinction shipped as a false
claim once: `sdkClientIdentifiers` read only top-level `ImportDeclaration`s, so
24 `await import('@aws-sdk/client-*')` sites had no verdict at all -- not a gap,
nothing -- including `deploy.ts`'s `GetCallerIdentity` on every deploy. So
`aws-client-defaults-fence.test.ts` reconciles the AST walk against a TEXT SCAN
for the construction itself, and every hit must be a site the walk also found or
a line of prose. Reverting the destructured-dynamic arm makes it red with 18
unexplained sites. **One shape the two methods get wrong the SAME way**, which is
the bound on the premise: both key on the `Client` suffix, so an aggregated
client (`import { S3 } from '@aws-sdk/client-s3'`) is invisible to both. Nothing
in the tree uses one.

**The allow-list may only SHRINK** — a STALE entry (a file that is now clean) and a DEAD one (a file with no client site) both fail, so finishing a file forces its entry out. PR 1 migrated `aws-clients.ts`, the bootstrap path and the state-write path and left 61 files listed; PR 2 empties it. Defences: FLOORS on total sites / files with sites / clean sites, so a collapsed parse fails loudly instead of reporting zero gaps; a `--src-dir=` seam (rejected outside `--check`) so the SHIPPED command's exit code is probed against a scratch COPY of the tree rather than by calling the exported helpers; and REAL-CODE regression probes for each verdict — a migrated file losing its defaults, the defaults moved out of first position, and an allow-listed file becoming clean. NO AWS integ (pure static analysis).
