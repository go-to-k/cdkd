...
5. Write tests
6. Add the resource type to [docs/supported-resources.md](../../docs/supported-resources.md) — one bullet under its category's H3 in `## Resource types`, alphabetical by category — AND to [docs/import.md](../../docs/import.md) (import-side coverage: auto-lookup vs override-only vs sub-resource). `tests/unit/scripts/provider-docs-coverage.test.ts` fails CI if either is missing, and it matches on the type STRING, so the shape of the list is free to change but the exact `AWS::Service::Type` spelling must appear.
7. **If the provider gates a stabilization wait on `process.env['CDKD_NO_WAIT']`** (i.e. `--no-wait` skips a multi-minute poll for this type), add the resource type to the `--no-wait` docs in ALL of: the per-type wait-semantics table + its intro in [docs/cli-deploy.md](../../docs/cli-deploy.md), and the `noWaitOption` help string + JSDoc in [src/cli/options.ts](../../src/cli/options.ts). (The README no longer enumerates flags — its former `--no-wait` feature bullet's per-type list lives in that docs/cli-deploy.md table, published at https://cdkd.dev/cli-deploy/.) Enforced by `tests/unit/provisioning/no-wait-doc-coverage.test.ts` (fails CI if a `CDKD_NO_WAIT`-honoring provider has no handled type in the cli-deploy.md table). The `AWS::Lambda::MicrovmImage` provider shipped honoring `--no-wait` but missed this list — the test is the backstop.
