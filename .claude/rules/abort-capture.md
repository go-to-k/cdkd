...
read as an unexplained abort and cost the lane a re-run. Nine `local-*`
fixtures carried it at 29 sites, plus three retry loops that lost every
attempt's stderr the same way.
...
It takes the exit status explicitly, prints the status and the tail of the
captured stderr on a non-zero exit, and still emits the last stdout line so
the assertion runs, FAILS, and prints its own diagnostic. Its stderr file is
per call and removed inside the helper, so the fixture's EXIT trap chain needs
no entry for it.
