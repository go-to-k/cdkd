...
- Ref: References another resource. Resolves to the CFn `Ref` value — the physicalId for most types; see `cfnRefValueFromPhysicalId` in `src/deployment/intrinsic-function-resolver.ts` for the exceptions (compound `<parent>|<child>` CC ids, ARN-stored SDK ids like `AWS::Events::Rule` / `AWS::CloudTrail::Trail` whose `Ref` is the name). A few exceptions recover the value from a STATE KEY instead (`TableName` / `SelectionId` / `RepositoryId` / the AppSync ARNs) via `refStateLookupFromResource` — that lookup REFUSES a leaf carrying `SECRET_MASK` and reports it through a callback EVERY caller must pass. Ignoring the callback is not an option and the type's optionality is not a licence: the fall-through emits the raw physical id, which no guard recognises, where the pre-#2847 `'***'` was recognised by four. `cdkd deploy` turns the report into a `redactedAttributeReads` refusal ([layout-deployment-secrets.md](layout-deployment-secrets.md)); `cdkd orphan`, which has no resolver context, reports the site as `unresolvable` and, under `--force`, warns and substitutes the physical id
