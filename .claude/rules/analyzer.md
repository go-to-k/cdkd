...
- Ref: References another resource. Resolves to the CFn `Ref` value — the physicalId for most types; see `cfnRefValueFromPhysicalId` in `src/deployment/intrinsic-function-resolver.ts` for the exceptions (compound `<parent>|<child>` CC ids, ARN-stored SDK ids like `AWS::Events::Rule` / `AWS::CloudTrail::Trail` whose `Ref` is the name). A few exceptions recover the value from a STATE KEY instead (`TableName` / `SelectionId` / `RepositoryId` / the AppSync ARNs) via `refStateLookupFromResource` — that lookup REFUSES a leaf carrying `SECRET_MASK` and reports it through a callback EVERY caller must pass AND ACT ON. Receiving it is not enough: the fall-through emits the raw physical id, which no guard recognises, where the pre-#2847 `'***'` was recognised by four, so a caller that takes it silently is worse than the bug. Three permitted buckets, and two review rounds each found a caller outside them: consult `redactedAttributeReads` and refuse (`cdkd deploy`'s CREATE / UPDATE arms AND its Outputs pass — see [layout-deployment-secrets.md](layout-deployment-secrets.md)); emit `SECRET_MASK` so a downstream reader catches it (`cdkd orphan --force`); or be provably a display/filter path. `cdkd orphan` without `--force` reports the site as `unresolvable` and aborts. The seam's own doc comment is the authority for the bucket list
