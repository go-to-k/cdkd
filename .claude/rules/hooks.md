...
**The relaxation also asks what gh would resolve in the TARGET CHECKOUT, not
only what the command says** (go-to-k/cdkd#3235): gh picks a base repo from that
checkout's remotes, so an `upstream` pointing elsewhere let one repo's marker
clear another repo's merge. What it resolves, why it is a comparison rather
than a presence test, the two fail-opens that shape it, and the second-clone
bound it retires are in
[hooks-target-repo-identity.md](hooks-target-repo-identity.md), which loads
when you touch that gate or its suite.
