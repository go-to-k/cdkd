...
- **Throw on CREATE, WARN on a state-borne UPDATE**: `rollback-executor.ts` replays `update()` with a STATE record as the desired bag (`replayingState`), as does `drift --revert` with a readback (`desiredFromAwsReadback`), and a refusal there leaves the resource un-rollbackable. A template-path update may refuse a template-borne value, before its first AWS call ([#3728](https://github.com/go-to-k/cdkd/issues/3728)).
