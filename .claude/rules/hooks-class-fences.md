...
the then-current `pr-body-item-number` gate, whose whole subject was a third
party's issue. The remedy follows this section's own rule rather
...
- **Some hooks are exercised by no literal control**, and the set is a DECLARED list rather than a count this sentence would drift against — read `DECLARED_UNEXERCISED` in the suite. The recurring reasons: a hook that refuses nothing by design (`restore-backup`, the two PostToolUse detectors), one whose matcher is not `Bash` at all (`worktree-owner-gate`), and one whose verb is not a git/gh verb (`broad-process-kill-gate`). The exclusion is checked as a PARTITION — registered == exercised + declared — in BOTH directions, so a new hook with no control fails AND a declared row naming a hook that no longer exists fails.
