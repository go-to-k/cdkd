...
**Remaining known gaps**, measured (plus #2210 / #2334; go-to-k/cdkd#2333 left ONE narrower residue, recorded on `gate_dequote_structural`: an UNENUMERATED value-consuming global flag makes its VALUE look like the subcommand, so a quoted verb one token later is not dequoted —
`git --unknown-opt someval "commit"` matches nothing while its literal-verb twin matches):
