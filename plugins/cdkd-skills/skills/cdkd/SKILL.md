...
cdkd resolves CloudFormation dynamic references (`{{resolve:secretsmanager:...}}`) and, when persisting state, stores the UNRESOLVED expression rather than the resolved plaintext — the value does not land in the `state.json` cdkd writes, nor in `cdkd state show`, `cdkd diff`, or `cdkd drift` output. A normal `cdkd deploy` already scrubs state this way as a side effect.
