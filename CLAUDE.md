...
interface StackState {
  ...
  skippedOutputs?: Record<string, string>; // informational, no bump (#2740): Outputs keys the last deploy SKIPPED → digest of their template inputs; `cdkd diff` previews such a key as absent only while its digest still holds
...
- **Before merging any PR that touches deletion logic**: A fourth markgate gate, `integ-destroy`, guards `gh pr merge` via `.claude/hooks/integ-destroy-gate.sh`. **Its scope is the `integ-destroy.include` list in `.markgate.yml`** — read it there; this file deliberately keeps no copy. **The gate has two halves and they must name the same files** — that `include` decides what stales the MARKER, the hook's activation patterns decide whether `gh pr merge` consults it at all: include-only is an invalidated marker no hook reads, hook-only is a FAIL-OPEN. Both directions are fenced by `tests/unit/scripts/cross-cutting-list-sync.test.ts`. Plus a **14-day wall-clock TTL** — real-AWS behavior drifts even when the repo doesn't. The marker uses markgate 0.4+'s `hash: diff` mode (`base: origin/main`), so another lane's merged in-scope change does not invalidate it while this branch's — committed or not — still does; the cross-file trade is bounded by the TTL. Only `/run-integ` sets it, and only when the destroy step finished with 0 errors AND the post-destroy AWS state was empty — so a PR whose destroy path has not been verified against real AWS recently is **physically unmergeable**.

- **Before merging any PR that touches cross-cutting deploy/destroy code**: A markgate gate, `integ-broad`, guards `gh pr merge` via `.claude/hooks/integ-broad-gate.sh`. **Its scope is `CROSS_CUTTING_REGEX` in that hook** — the executable copy, mirrored in `/verify-pr` step 6 and `/pick-integ` step 2 and fenced against the hook by `tests/unit/scripts/cross-cutting-list-sync.test.ts`; this file keeps no copy. Plus the same **14-day wall-clock TTL** as `integ-destroy` / `integ-local`. Why a separate gate: `integ-destroy` accepts ANY clean real-AWS destroy, but cross-cutting code affects multi-resource VPC / Lambda / Custom-Resource paths a narrow feature integ never exercises. The marker is bound to a sentinel file `.markgate-broad-integ-test` that `/run-integ` updates ONLY when the test name is in the broad set (enumerated in the hook's block message, and in `/run-integ` step 11) AND the run was clean — so a narrow feature integ legitimately flips `integ-destroy` while leaving `integ-broad` stale, exactly the gradient intended.
