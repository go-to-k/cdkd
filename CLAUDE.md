...
- ✅ `cdkd drift` single-stack auto-detect + `getDriftUnknownPaths` for unreadable state keys: positional stack name is now optional — when state has exactly one stack and `--all` is not set, the command auto-selects it (mirrors `cdkd deploy` / `cdkd destroy`); state with zero or multiple stacks reports a clear error with the available list. Separately, `ResourceProvider.getDriftUnknownPaths?(): string[]` lets a provider declare state property paths it deliberately cannot read back from AWS — `calculateResourceDrift` skips those paths so they don't fire guaranteed false-positive drift on every clean run. First user: `LambdaFunctionProvider` returns `['Code']` (`GetFunction` only exposes a pre-signed URL, never the original `S3Bucket` / `S3Key`). Other providers documented as "intentionally omitted" in their `readCurrentState` (`Lambda::LayerVersion.Content`, `SecretsManager::Secret.SecretString`, `IAM::Role.Policies` inline bodies, `SNS::Topic.DeliveryStatusLogging`) can opt in incrementally — the comparator infrastructure now exists, but each provider has to declare its own paths. Implementation: `src/types/resource.ts` (interface) + `src/analyzer/drift-calculator.ts` (`ignorePaths` option) + `src/cli/commands/drift.ts` (auto-detect + per-resource `getDriftUnknownPaths` plumbing) + `src/provisioning/providers/lambda-function-provider.ts` (`['Code']`).
