...
- **Registration is not execution — prove the gates are ALIVE before the first commit of a session**: run `git commit --dry-run -m "gate liveness probe"` from the repo root **as a Bash TOOL CALL** (PreToolUse hooks gate the AGENT's tool calls only; a human-typed line proves nothing). `--dry-run` commits nothing; a `Blocked by branch-gate` / `Blocked by check-gate` line means the hooks fire; git's ordinary output means they do not (the sibling repos spent a day with every gate registered and inert — go-to-k/cdk-real-drift#1801; `/hooks` lists registration, not firing). **A gate with nothing to trip proves nothing**, and the plain probe is deterministic, so re-running it settles nothing either: with the markers already fresh it returns ordinary output forever. Read it beside `mise exec -- markgate verify check` AND `verify docs` (either at rc=1 — or rc=2, which markgate uses for "could not evaluate" — means that ordinary output is a FAIL-OPEN, not a fresh marker), or probe by SHAPE: a `git commit -m "$(cat <<'EOF' ... EOF)"` form trips `commit-msg-heredoc-gate` on any branch under any marker state (measured 2026-09-05, rc=2; a STALE `check` marker masks it, so it is the probe to use when markers are fresh). **Until one of the two is CONCLUSIVE, treat every gate as self-enforced and run its check by hand.**
