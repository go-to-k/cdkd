...
- **Before every commit**: Two markgate gates guard `git commit` via `.claude/hooks/check-gate.sh`. Both must be fresh:
  - `check` — recorded by `/check` (typecheck, lint, build, tests). Scope: `src/**`, `tests/**`, `scripts/**` (the CI classifier / codegen logic — issue #1592), the four verification-depth skills under `.claude/skills/`, build/test configs (see `.markgate.yml`). Only invalidated by changes in that scope.
  ...
  **Run the required skills proactively** before attempting the commit — look at `git status` / `git diff --cached --name-only` and match it against each gate's scope: a tests-only commit only needs `/check`; a docs-only commit only needs `/check-docs` (which now also includes `.claude/rules/**`); a src edit needs both; changes that fall outside both scopes (e.g. `.claude/hooks/**`, `.markgate.yml`) need neither — but note the four verification-depth skills (`.claude/skills/{review-pr,pick-integ,run-integ,verify-pr}/**`) ARE in the `check` scope, since a checker's subject is exactly what those files say. The hook is a safety net, not the primary trigger — if you see "Blocked by check-gate", the message names exactly which skill to re-run, but getting there means you skipped the proactive step. `/verify-pr` refreshes both markers in one shot. Install `vp` and markgate via `mise install` at the repo root (see CONTRIBUTING.md).
...
- **Every session-wrap / task-complete report MUST end with a "Remaining work" section, a "State" line, AND a "Session close" verdict — unprompted**: the user should never have to ask "any follow-up tasks?", "did you stop or are you waiting?", or "can I close this session?".
  ...
  **Session close** — a one-line verdict: **CLOSEABLE** or **NOT CLOSEABLE (waiting on: ...)** naming the blocker. CLOSEABLE requires ALL of: working tree clean and not on a feature branch left dangling; no open PRs owned by this session; no running background tasks / integs / subagents; no AWS resources pending cleanup; every TODO filed as an issue; and **zero `Session-fit: now` TODOs left open**.
  ...
  When the verdict is CLOSEABLE and `next` TODOs exist, close the report with a **handoff line** — the literal command that starts the next session, so resuming costs no thought: e.g. `Next session: /work-issues` or `Next session: fix issue #1791 (Effort: M)`. Group the `next` items by whether one fresh session could take them together (file-disjoint lanes) or whether they must be serialized, and say which. A `next` item without a handoff line is the same hesitation this classification exists to remove, just moved to the end of the report. If any of these is unmet, the verdict is NOT CLOSEABLE and names the blocker. When the state is WAITING, the thing being awaited IS the blocker — the two lines must name the same thing, not diverge. A report that ends without all three of the Remaining-work section, the State line, and the Session-close verdict is incomplete.
