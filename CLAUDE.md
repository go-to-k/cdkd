...
- **[docs/cli-reference.md](docs/cli-reference.md)** - CLI reference overview (output streams, `--region`, `--role-arn`, exit codes) + index of the per-command reference pages (`docs/cli-deploy.md`, `docs/cli-deploy-safety.md`, `docs/cli-bootstrap-gc.md`, `docs/cli-diff.md`, `docs/cli-drift.md`, `docs/cli-destroy.md`, `docs/cli-rollback.md`, `docs/cli-export.md`, `docs/cli-scrub.md`, `docs/cli-publish-assets.md`, `docs/cli-events.md`). The per-resource-type **wait-semantics table** (`--no-wait` / default / `--full-wait` next to CloudFormation and Terraform) lives in [docs/cli-deploy.md](docs/cli-deploy.md). cdkd is template-compatible with CloudFormation but NOT wait-semantics-identical; that table is the single source of truth for what "done" means per type
...
- **Never commit or push directly to `main`**: All changes must land via a feature branch + PR. Feature work must live in its OWN worktree under `.claude/worktrees/<branch>/` — DO NOT branch in the main worktree (`/Users/goto/pc/github/cdkd` itself). The main tree is a shared resource across parallel agents; the `main-tree-branch-gate.sh` hook physically blocks `git switch -c <branch>` / `git switch <feat>` / `git checkout -b <branch>` in the main tree, and since 2026-09-01 reads its arguments with a real option parse over git's ARGV rather than "token 1 and token 2" — so the glued spellings (`-bfeat`, `--orphan=feat`), an unambiguous PREFIX of a long name (`--orph`, `--trac`), a flag ahead of the branch (`git checkout -f <feat>`), a redirection or a trailing `# comment` beside the branch, a `--` with nothing after it, a branch that exists only on a CONFIGURED remote, `-t origin/<b>`, and `-` / `@{-1}` are blocked too, while the file restores (`git checkout <branch> -- <paths>`, the same without the `--`, `-p` / `--ours` / `--theirs` / `--pathspec-from-file`), `git switch -- main`, `git checkout --no-guess <remote-only-name>` and `--help` pass. See [.claude/rules/hooks-main-tree-branch.md](.claude/rules/hooks-main-tree-branch.md) for the measured before/after tables. Correct invocation: `git worktree add .claude/worktrees/<branch> -b <branch> origin/main && cd .claude/worktrees/<branch>`, do the work, then `git worktree remove .claude/worktrees/<branch>` when done. **That recipe is the MAIN-CHECKOUT case, and it is wrong from anywhere else** (issue #2390): when the session is ALREADY inside a linked worktree -- an Orca/ADE workspace, or a stray `cd` into an existing lane -- `git worktree add` NESTS one worktree inside another, and deleting the outer workspace takes the inner directory, its uncommitted work and its git registration with it. There, create no WORKTREE and remove none -- but you still take a BRANCH, in the tree you are standing in: never commit onto the branch it was handed to you on, because that branch belongs to the outer tool and `gh pr merge --delete-branch` would delete its remote on the way out. At the end, switch back to that branch AS-IS (no pull, no rebase, no fast-forward), delete only the branch you created, and leave the tree for whoever made it (issue #2417). `/work-issues` computes which case applies before its first stage and `/hunt-bugs` points at that probe; do not re-implement it here. The `branch-gate.sh` hook ALSO blocks `git commit` / `git push` when the target git working tree is on `main` / `master` — **and, since issue #2402, when the MAIN checkout is on a DETACHED HEAD**, which reads as "not on a feature branch" rather than as "a tree the gate cannot see". That closes the two-gate composition where `git checkout <sha>` (passed by main-tree-branch-gate as inspection) detached the shared tree and left the commit ungated; a detached LINKED worktree still passes, because that is the lane-clearing remedy the Stop hook prescribes. Defense-in-depth — main-tree-branch-gate prevents the cause, branch-gate catches the symptom. The `post-merge-orphan-push-gate.sh` hook blocks pushing to a branch whose PR has already merged. See [.claude/rules/hooks.md](.claude/rules/hooks.md) for the per-hook details.
