...
- **src/cli/** - CLI command implementations (deploy, destroy, diff, synth, list/ls, bootstrap, force-unlock, import, state), config resolution.
  ...
  **`cdkd import` vs upstream `cdk import` — parity notes** (see [docs/import.md](docs/import.md) for the full matrix; this is a quick checklist when working on the import code path):
  ...
  - **`--migrate-from-cloudformation [name]` is cdkd-specific.** End-to-end migration off CloudFormation: pre-import `DescribeStackResources` to recover physical IDs (so cdk-deployed stacks work without `--resource`) → import → state write → post-import `UpdateStack` (inject Retain) → `DeleteStack`. No upstream equivalent — `cdk import` only adopts resources INTO a CFn stack, never out of one. JSON-only, inline-`TemplateBody`-only, incompatible with `--dry-run` (see the import section above for the full constraint list).
...
- ✅ CLI: `cdkd import --migrate-from-cloudformation [cfn-stack-name]` — end-to-end migration off CloudFormation in a single command. Pre-import: `DescribeStackResources` recovers `(logicalId, physicalId)` pairs from the source CFn stack and merges them into the import overrides — so `cdk deploy`-managed stacks (which don't propagate `aws:cdk:path` as an AWS tag) work without per-resource `--resource` flags. The flag also forces auto mode (CFn-derived overrides don't trigger selective mode). Post-state-write: runs the AWS-recommended retirement (`DescribeStacks` → `GetTemplate` Original-stage → inject `DeletionPolicy: Retain` + `UpdateReplacePolicy: Retain` → `UpdateStack` → `DeleteStack`) so the source CFn stack is retired without deleting the underlying AWS resources. JSON-only (CDK-generated templates); inline `TemplateBody` only (51,200-byte limit; over-limit templates fail with a pointer to the manual 3-step procedure, cdkd state is already written so the user can re-run or finish manually); incompatible with `--dry-run`. Helpers at `src/cli/commands/retire-cfn-stack.ts` (`getCloudFormationResourceMapping` + `retireCloudFormationStack`)
...
- ✅ Lambda CREATE / UPDATE waits for `Configuration.State === 'Active'` (post-CreateFunction, via SDK `waitUntilFunctionActiveV2`) and `LastUpdateStatus === 'Successful'` (after each UpdateFunctionConfiguration / UpdateFunctionCode, via SDK `waitUntilFunctionUpdatedV2`). CreateFunction returns synchronously while the function is still in `Pending`; without this wait a Lambda-backed Custom Resource that depends on the function fails with "The function is currently in the following state: Pending" because cdkd had already marked the resource as `created` and dispatched the downstream Invoke. CFn-parity behavior. Both waits are intentionally NOT gated on `--no-wait` / `CDKD_NO_WAIT` — those skip *convenience* waits for resources that don't block siblings (CloudFront propagation, RDS warm-up); a Pending Lambda DOES block siblings and is therefore required for correctness, not just polish. Bounded by a 10-minute waiter timeout and wrapped as `ProvisioningError` on FAILURE/TIMEOUT so the deploy engine's standard rollback path runs unchanged. The pre-delete `waitForLambdaUpdateCompleted` helper is intentionally separate from these two — it accepts ANY non-`InProgress` LastUpdateStatus (including `Failed`) because the function is being deleted anyway and a prior failed update should not block the delete.
