...
- ✅ `cdkd drift <stack>`: compares the `properties` recorded in cdkd state against the AWS-current snapshot returned by each provider's optional `readCurrentState` method. **CC API fallback runs automatically** (PR F) — when an SDK Provider has no first-class `readCurrentState` of its own, the drift command resolves the CC API provider via `providerRegistry.getCloudControlProvider()` and calls its generic `GetResource`-based `readCurrentState`, so SDK-Provider-managed resource types without a dedicated drift impl still get meaningful drift out of the box. CC API fallback covers the majority of resource types via `GetResource`; first-class `readCurrentState` is also implemented across four batches for shape precision. **Batch 0 (PR D, 7 high-traffic types)**: `LambdaFunctionProvider` (via `GetFunction`), `S3BucketProvider` (via `GetBucketVersioning` + `GetBucketEncryption` + `GetPublicAccessBlock` + `GetBucketTagging`), `DynamoDBTableProvider` (via `DescribeTable`), `IAMRoleProvider` (via `GetRole` + `ListAttachedRolePolicies`; AssumeRolePolicyDocument is URL-decoded + JSON-parsed back to the object form cdkd state holds), `SQSQueueProvider` (via `GetQueueAttributes`; numeric / boolean strings type-coerced, RedrivePolicy JSON-parsed), `SNSTopicProvider` (via `GetTopicAttributes`; same type-coercion pattern), and `LogsLogGroupProvider` (via `DescribeLogGroups` filtered by exact name; camelCase API → PascalCase CFn property names). **Batch 1 (13 providers / 19 resource types)**: `CloudFrontOAIProvider`, `EventBridgeBusProvider`, `EventBridgeRuleProvider` (+ `ListTargetsByRule`), `SSMParameterProvider`, `SecretsManagerSecretProvider` (SecretString deliberately not surfaced), `ECRProvider` (+ `GetLifecyclePolicy`; per-feature NotFound caught), `StepFunctionsProvider` (Definition JSON-parsed), `ECSProvider` (multi-type: Cluster / Service / TaskDefinition; Service physicalId composite `<clusterArn>|<serviceName>`), `RDSProvider` (multi-type: DBInstance / DBCluster / DBSubnetGroup), `KMSProvider` (multi-type: Key + Alias), `ApiGatewayProvider` (Account + Method; sub-resources skipped), `ApiGatewayV2Provider` (Api), `CognitoUserPoolProvider`. **Batch 2 (14 mid-traffic providers)**: `AppSyncProvider` (multi-type: GraphqlApi / DataSource / Resolver / ApiKey; schema bodies out of scope), `EFSProvider` (multi-call), `ElastiCacheProvider` (multi-type: CacheCluster / CacheSubnetGroup), `ELBv2Provider` (multi-type: LoadBalancer / TargetGroup / Listener), `Route53Provider` (multi-type: HostedZone / RecordSet), `WAFv2WebACLProvider`, `FirehoseProvider` (destination configurations skipped due to shape divergence), `KinesisStreamProvider`, `GlueProvider` (multi-type: Database / Table), `CloudTrailProvider` (multi-call), `CloudWatchAlarmProvider`, `CodeBuildProvider` (camelCase → PascalCase remap), `ServiceDiscoveryProvider` (multi-type: PrivateDnsNamespace / Service), `SNSSubscriptionProvider`. **Batch 3 (this PR, 12 sub-resource + special providers covering ~31 resource types)**: `IAMInstanceProfileProvider`, `IAMUserGroupProvider` (multi-type: User + Group; UserToGroupAddition deliberately skipped — membership-only with no AWS-side identity), `LambdaEventSourceMappingProvider` (Enabled derived from `State`), `LambdaLayerVersionProvider` (LayerName parsed from ARN; `Content` omitted because GetLayerVersion returns a pre-signed URL, not the asset hash state holds), `LambdaUrlProvider` (Cors keys re-shaped to `create()`'s shape), `S3BucketPolicyProvider` (via `GetBucketPolicy`; PolicyDocument JSON-parsed), `SNSTopicPolicyProvider` (parses comma-joined topic ARNs from physical id, fetches `GetTopicAttributes(Policy)` on the first topic), `SQSQueuePolicyProvider` (via `GetQueueAttributes(Policy)`), `S3DirectoryBucketProvider` (`HeadBucket` + `--x-s3` suffix detection for `DataRedundancy`), `S3TablesProvider` (multi-type: TableBucket / Namespace / Table — Namespace surfaces TableBucketARN+Namespace from the parsed physical id with no SDK call), `S3VectorsProvider` (camelCase → PascalCase re-shape, including `sseType` → `SSEType` / `kmsKeyArn` → `KMSKeyArn`), `AgentCoreRuntimeProvider` (recursive camelCase → PascalCase via `camelToPascalCaseKeys`; ProtocolConfiguration surfaced in object form), `EC2Provider` (multi-type dispatch — VPC / Subnet / InternetGateway / NatGateway / RouteTable / SecurityGroup / Instance / NetworkAcl). **SDK-side first-class skips with documented reasons** (PR F's CC API fallback now covers the majority of these automatically): `CloudFrontDistributionProvider` (Batch 1 — DistributionConfig SDK shape `Quantity + Items` vs CFn flat-array shape; CC API fallback now answers); `IAMPolicyProvider` (Batch 3 — inline-policy attachment); `LambdaPermissionProvider` (Batch 3 — statement-inside-policy); EC2 sub-resources `Route` / `SubnetRouteTableAssociation` / `SecurityGroupIngress` / `NetworkAclEntry` / `SubnetNetworkAclAssociation` / `VPCGatewayAttachment` (rule / association sub-resources surfaced inside parent lists). The IAMPolicy / LambdaPermission skips and API Gateway sub-resource skips share a root cause: the `readCurrentState(physicalId, logicalId, resourceType)` signature does not pass `properties`, so any sub-resource whose Describe API needs a parent identifier (RestApiId, FunctionName, target Role list, etc.) cannot recover it via a first-class SDK impl. A signature extension is the canonical follow-up. Each implementation mirrors what the same provider's `create()` accepts so the drift comparator only sees keys that are actually managed; AWS-managed fields (timestamps, FunctionArn, RevisionId, etc.) are filtered at the wire layer. Tag drift is deliberately out of scope for v1 (the `aws:cdk:path` auto-tag warrants a separate filter-strategy decision). Per-resource outcomes: `drifted` (one or more property paths differ — printed as `~ <logicalId>` with `+/-` lines), `clean` (state matches AWS), `unsupported` (BOTH the SDK Provider AND the CC API fallback failed to answer — printed as `? <logicalId>` so users see what's still uncovered). Exits **0** when no drift, **1** when drift detected (signalled via a `silent: true` `DriftDetectedError` so the rich human report is the only output) OR command crashed (default error handler — same as every other cdkd command). State-driven (no synth needed) — flag set mirrors `state show`: `--all`, `--stack-region`, `--state-bucket`, `--state-prefix`, `--json`, `--profile`, `--verbose`. Comparator only descends into keys present in cdkd state, so AWS-managed fields (timestamps, generated identifiers, account-wide defaults) cannot fire false-positive drift. **Drift resolution** (mutually exclusive with each other; both honor `--dry-run`): `--accept` writes the AWS-current values back into cdkd state under the per-stack lock with `IfMatch` optimistic locking on the captured ETag (state ← AWS — use when a console edit is the intentional source of truth); `--revert` calls each drifted resource's `provider.update(logicalId, physicalId, type, stateProps /*new*/, awsProps /*old*/)` under the same lock, with the AWS-current snapshot reused from the drift read so no second read fires (AWS ← state — use to undo a console change). `--revert` failures are collected per-resource and surface as `PartialFailureError` (exit 2); state is NOT updated by `--revert` (AWS values match state once update succeeds). `-y/--yes` skips the confirm prompt; `--concurrency <n>` (default 4) caps `--revert`'s parallel `provider.update` calls. Implementation: `src/cli/commands/drift.ts` + `src/analyzer/drift-calculator.ts`; CC-API `readCurrentState` in `src/provisioning/cloud-control-provider.ts`; SDK provider `readCurrentState` impls under `src/provisioning/providers/`.
