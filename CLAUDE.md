...
- **Before every commit**: Two markgate gates guard `git commit` via `.claude/hooks/check-gate.sh`. Both must be fresh:
  - `check` — recorded by `/check` (typecheck, lint, build, tests). Scope: `src/**`, `tests/**`, `scripts/**` (the CI classifier / codegen logic — issue #1592), the four verification-depth skills under `.claude/skills/`, build/test configs, and `.mise.toml` (see `.markgate.yml`). Only invalidated by changes in that scope. `.mise.toml` is in scope because it pins the `vp` binary that RUNS the build/test definitions — a toolchain bump changes what green means, so a marker recorded under the old one must not keep reporting fresh (issue #1954).
