...
- ✅ IAM Role drift now covers inline `Policies` and `PermissionsBoundary`: closes two gaps in `IAMRoleProvider.readCurrentState`. **Inline policies** — previously declared in `getDriftUnknownPaths(): ['Policies']` and skipped by the comparator (the v1 punt: surfacing names without bodies fires guaranteed false-positive drift, fetching every body costs one extra `GetRolePolicy` per inline policy). Now read in via `ListRolePolicies` (paginated for forward-compat) + `Promise.all` over `GetRolePolicy` per name (capped at IAM's 10-per-role limit), URL-decoded + JSON-parsed. Order is reconciled against state's `Policies` array (when supplied via the optional `properties` parameter): state-listed names come first in their original order, AWS-only names (added via console) tail-append in lexicographic order — so a clean run produces zero positional-compare drift, and console-side adds still surface as length / content mismatch. `getDriftUnknownPaths()` is removed entirely. **PermissionsBoundary** — previously gated by `if (role.PermissionsBoundary?.PermissionsBoundaryArn !== undefined)` so a console-side ADD on a role deployed without a boundary stayed invisible to the drift comparator (top-level walk is state-keys-only). Now follows the PR #145 always-emit pattern: `result['PermissionsBoundary'] = role.PermissionsBoundary?.PermissionsBoundaryArn ?? '';` so observedProperties always carries the key and the comparator descends into it. The existing `update()` flow already handles `''`-vs-arn correctly (`'' !== oldBoundary` → `DeleteRolePermissionsBoundary` via the existing else-if branch), so `cdkd drift --revert` correctly removes a console-added boundary. Implementation in [src/provisioning/providers/iam-role-provider.ts](src/provisioning/providers/iam-role-provider.ts); 5 new readCurrentState tests cover always-emit placeholder, inline policy round-trip, state-driven order reconciliation, and AWS-only tail-append.
