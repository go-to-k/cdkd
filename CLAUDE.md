...
- **Every session-wrap / task-complete report MUST include a "Follow-ups" section — unprompted**: the user should never have to ask "any follow-up tasks?". Classify every residual into one of three buckets and say which: (a) **filed** — a doable-but-deferred item, with its GitHub issue number (file the issue BEFORE reporting, in the same turn the deferral is decided); (b) **accepted** — a known cost consciously shipped, naming where the acceptance is recorded (PR body, issue comment); (c) **none** — an explicit "follow-ups: none" statement after actually auditing for parity gaps, deferred polish, and accepted review nits. A report that ends without one of these three is incomplete.
